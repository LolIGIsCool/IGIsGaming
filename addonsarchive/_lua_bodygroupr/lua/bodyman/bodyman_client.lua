...
CreateClientConVar( "bodyman_currentplayermodel", "0", true, false )
...
--	pmodel:SetModel( LocalPlayer():GetModel() )
	pmodel:SetModel( GetConVar( "bodyman_currentplayermodel" ):GetString() ) -- Alex
				...
				if BODYMAN:HasBodyGroup( LocalPlayer(), bgname, i ) then
					...
					function btn:DoClick()
						...
						if bgid ~= -1 then
							...
							net.Start("bodygroups_changemooseman")
...
concommand.Add("bodyman_openmenu", function()
	LocalPlayer():ConCommand( "bodyman_getmodel")
	timer.Simple( 0.1, function() -- need some time for the server sided model name
		BODYMAN:OpenMenu()
	end)
end) -- Alex
