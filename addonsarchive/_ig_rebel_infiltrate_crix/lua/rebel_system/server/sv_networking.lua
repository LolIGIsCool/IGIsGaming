...
local filterBlacklistedPlayers = function()
	...
	if table.Count(playerList) < 20 then return nil end -- change to 20 before release
	local newList = {}
	local count = 0
	for k, v in ipairs(playerList) do
		...
		local id = v:SteamID()
		if not ( MISSIONS.Blacklist[id] or MISSIONS.Completed[id] ) and not (disallowedRegs[v:GetRegiment()]) and ( clearance >1 and clearance < 6) then 
			count = count + 1	
			newList[count] = v
		end
	end
