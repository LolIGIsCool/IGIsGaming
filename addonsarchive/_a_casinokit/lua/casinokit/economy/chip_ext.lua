...
if CLIENT then
	...
	net.Receive("ckit_chiptransaction", function()
		...
		local frompos = IsEntity(from) and from:WorldSpaceCenter() or from
		...
		hook.Add("PostDrawTranslucentRenderables", hid, function()
			if progress >= 1 or (IsEntity(to) and not IsValid(to)) then
				hook.Remove("PostDrawOpaqueRenderables", hid)
				return
			end

			local fromp, top = frompos, IsEntity(to) and to:WorldSpaceCenter() or to
			if EyePos():Distance(fromp) > 256 or EyePos():Distance(top) > 256 then
				return
			end
			...
			for i,info in pairs(chipdt) do
				...
				if lprog < 1 then
					local lpos = LerpVector(leprog, fromp, top) + Vector(((i*0.6)%1) * 3 + math.sin(i+CurTime()) * 2, ((0.5+i*0.6)%1) * 3 + math.cos(i+CurTime()) * 2, math.sin(leprog * math.pi) * 20)
