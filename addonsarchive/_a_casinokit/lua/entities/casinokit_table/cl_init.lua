...
function ENT:DrawTableModel()
	...
	self.OverlayModel:SetMaterial("phoenix_storms/ps_grass")
...
local cvar_overviewYaw = CreateConVar("casinokit_overviewyaw", "0")
...
local function calcVehViewPAF(veh)
	...
	local lpos
	do
		local rad = cvar_overviewYaw:GetFloat()
		lpos = par.TableLocalOrigin + par:CalculateLocalHorizPosForAngle(rad, cvar_overviewZoom:GetFloat()) + Vector(0, 0, 20 + 60 * cvar_overviewZoom:GetFloat())
	end
...
local yawSpeed = 0
hook.Add("SetupMove", "CasinoKit_AboveView", function(ply, mv, cmd)
	local veh = ply:GetVehicle()
	if IsValid(veh) and veh:CKit_IsSeat() and veh:GetThirdPersonMode() then
		yawSpeed = yawSpeed * 0.99 + cmd:GetMouseX() * 0.000008

		cvar_overviewYaw:SetFloat((cvar_overviewYaw:GetFloat() + yawSpeed) % (math.pi * 2))
	end
end)
