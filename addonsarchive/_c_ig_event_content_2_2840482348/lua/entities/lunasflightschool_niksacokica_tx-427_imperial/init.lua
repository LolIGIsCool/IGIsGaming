...
function ENT:OnTick()
	...
	if IsValid( Driver ) then	
		...
		local entRoll = self:GetAngles().r
		if(entRoll > 90 || entRoll < -90) then
			MoveX = 0
			MoveY = 0
	...
	if self:HitGround() then return end
...
function ENT:Passengers()
	if IsValid( self.gunner ) then
		...
		if gunner ~= self:GetCoDriver() then
...
function ENT:FireTurretPrimary()
	...
	if Muzzle then		
		...
		local ent = ents.Create( "lfs_tx-427_main_imperial" )
		ent:SetPos( Muzzle.Pos )
		ent:SetAngles( (-Muzzle.Ang:Right()):Angle() )
		ent:Spawn()
		ent.attacker = self:GetCoDriver()
		ent.inflictor = self
...
function ENT:FireTurretSecondary()
	...
	if Muzzle then
		...
		bullet.HullSize 	= 2
		bullet.Damage	= 20
		bullet.Attacker 	= self:GetCoDriver()
...
function ENT:CoDriverWeapons( co_driver, Pod )
	...
	if co_driver:KeyDown( IN_ATTACK2 ) then
		self:FireRockets()
	end
end
...
function ENT:FireRockets()
	...
	for i = 0, 1 do
		for j = 0, 3 do
			for k = 0, 3 do
				table.insert( fp, ( flip and "missiles_right_muzzle_" or "missiles_left_muzzle_" ) .. j .. k )
				flip = !flip
			end
		end
	end
	...
	if self.NumR > 32 then self.NumR = 1
	elseif self.NumR == 32 then self.firer = CurTime() + 30 end
	
	local pos = self:GetAttachment( self:LookupAttachment( fp[self.NumR] ) ).Pos
	local ang = ( -self:GetAttachment( self:LookupAttachment( fp[self.NumR] ) ).Ang:Right() ):Angle()
	...
	local tr = util.TraceHull( {
		start = startpos,
		endpos = (startpos - self:GetAttachment( self:LookupAttachment( fp[self.NumR] ) ).Ang:Right() * 50000),
