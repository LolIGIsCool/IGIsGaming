...
local keypad_crack_time = CreateConVar("axl_keypad_crack_time", "30", {FCVAR_ARCHIVE}, "Seconds for keypad cracker to crack keypad")
...
SWEP.Author = "Lestrigon" // From Willox
...
function SWEP:PrimaryAttack()
	...
	if IsValid(ent) and tr.HitPos:DistToSqr(self.Owner:GetShootPos()) <= 50*50 and ent.IsAXLKeypad then
...
function SWEP:Succeed()
	...
	if SERVER and IsValid(ent) and tr.HitPos:DistToSqr(self.Owner:GetShootPos()) <= 50*50 and ent.IsAXLKeypad then

		ent.timeOut = SysTime();
		ent:EmitSound("npc/roller/mine/rmine_shockvehicle2.wav");
		ent:SetNWString("alert", "SERVICE DENIAL");
		ent:SetNWInt("alert_color", 4);
		ent:SetNWInt("locked", true);
...
function SWEP:Think()
	...
	if self.IsCracking and IsValid(self.Owner) then
		...
		if not IsValid(tr.Entity) or tr.HitPos:DistToSqr(self.Owner:GetShootPos()) > 50*50 or not tr.Entity.IsAXLKeypad then
			self:Fail()
...
if(CLIENT) then
	...
	net.Receive("KeypadCracker_Hold", function()
		...
		if IsValid(ent) and ent:IsWeapon() and ent:GetClass():lower() == "axl_keypad_cracker" and not game.SinglePlayer() and ent.SetWeaponHoldType then
			if not state then
