...
ENT.CallForBackUpOnDamageAnimation = {ACT_SIGNAL1} -- Animation used if the SNPC does the CallForBackUpOnDamage function
	-- To let the base automatically detect the animation duration, set this to false:
...
ENT.AnimTbl_WeaponReload = {ACT_RELOAD, ACT_RELOAD_PISTOL} -- Animations that play when the SNPC reloads
...
ENT.WaitForEnemyToComeOutTime2 = 0.1 -- How much time should it wait until it starts chasing the enemy? | Second number in math.random
ENT.WaitForEnemyToComeOutTime2 = 0.2 -- How much time should it wait until it starts chasing the enemy? | Second number in math.random
...
ENT.JetpackJump_Rangemax = 18000
ENT.JetpackJump_Range = 18000
ENT.JetpackJump_Rangeclose = 350
ENT.JetpackJump_Cooldown = 5
...
function ENT:CustomOnThink_AIEnabled()
	...
	*/
	...
	if self.SmartAttackPos_NextT < CurTime() and self.LookForCover_activated != 1 and self.SmartAttackPos_activated != 1 and self.FlankEnemy_NextT > CurTime() and self.vACT_StopAttacks != true and self:GetEnemy() != nil and self:IsMoving() != true and IsValid(self:GetEnemy()) and (self.CurrentSchedule == nil) then
	self.SmartAttackPos_NextT = CurTime() + self.SmartAttackPos_Var.Nexttime
			...
			if self:DoRelationshipCheck(vc) == false and (vc.vj_doi_isteam != nil and vc.vj_doi_isteam == self.vj_doi_isteam) and (IsValid(vc:GetEnemy()) and vc:GetEnemy() != nil and vc:Visible(vc:GetEnemy()) ) then
				...
				if vc.Weapon_TimeSinceLastShot <= 5 then
	...
	if self.BlindFire_activated == 0 and self.CornerCover_activated == 0 and self.ThrowingGrenade != true and self.vACT_StopAttacks != true and self.Flinching != true and self:GetActiveWeapon() != NULL and self:GetActiveWeapon() != nil and self:GetEnemy() != nil and math.random(1,10) >= self.BlindFire_Chance then
	if (self:GetActiveWeapon().NPC_NoMoveShoot == nil or self:GetActiveWeapon().NPC_NoMoveShoot != true) and self:Visible(self:GetEnemy()) and self:GetPos():DistToSqr( self:GetEnemy():GetPos() ) <= self.BlindFire_range*self.BlindFire_range and self:VJ_ForwardIsHidingZone(self:NearestPoint(self:GetPos() +self:OBBCenter()),self:GetEnemy():EyePos()) == true and self.Weapon_ShotsSinceLastReload < self.Weapon_StartingAmmoAmount*0.6 then
		...
		if self:GetActiveWeapon().HoldType != nil and self:GetActiveWeapon().HoldType == "pistol" then
	...
	if self:IsMoving() == false and math.random(1,10) >= self.Reloadwhenattacking_chance and self.SmartGrenadeThrow_enemyvisible_lastposition != Vector(0,0,0) and self.LastSeenEnemyTime > 3 and self.Reloadwhenidle_activated != 1 and self.Weapon_ShotsSinceLastReload >= self.Weapon_StartingAmmoAmount*0.35 then
		local foundents = 0
