...
------ Core Variables ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
ENT.Model = {"models/KriegSyntax/imperial/stormtroopers/stormtroopers/npc_male_01.mdl"} -- The game will pick a random model from the table when the SNPC is spawned | Add as many as you want
ENT.StartHealth = ("100")
...
ENT.AnimTbl_Medic_GiveHealth = {ACT_SIGNAL_FORWARD} -- Animations is plays when giving health to an ally
...
ENT.AnimTbl_TakingCover = {ACT_COVER} -- The animation it plays when hiding in a covered position, leave empty to let the base decide
...
ENT.AnimTbl_WeaponAttack = {ACT_RANGE_ATTACK_SMG1, ACT_RANGE_ATTACK_AR2} -- Animation played when the SNPC does weapon attack | For VJ Weapons
...
ENT.AnimTbl_WeaponAttackCrouch = {ACT_RANGE_AIM_AR2_LOW, ACT_RANGE_ATTACK_SMG1_LOW} -- Animation played when the SNPC does weapon attack while crouching | For VJ Weapons
ENT.CanCrouchOnWeaponAttackChance = 2.5 -- How much chance of crouching? | 1 = Crouch every time
ENT.AnimTbl_WeaponAttackFiringGesture = {ACT_GESTURE_RANGE_ATTACK_AR1, ACT_GESTURE_RANGE_ATTACK_AR2, ACT_GESTURE_RANGE_ATTACK_SMG1, ACT_GESTURE_RANGE_ATTACK_HMG1} -- Firing Gesture animations used when the SNPC is firing the weapon | Leave empty for the base to decide
...
ENT.AnimTbl_ShootWhileMovingRun = {ACT_RUN_AIM_RIFLE} -- Animations it will play when shooting while running | NOTE: Weapon may translate the animation that they see fit!
ENT.AnimTbl_ShootWhileMovingWalk = {ACT_WALK_AIM_RIFLE} -- Animations it will play when shooting while walking | NOTE: Weapon may translate the animation that they see fit!
	-- ====== Reloading Variables ====== --
...
function ENT:CustomOnInitialize()
	if math.random(1,0) == 1 then
		...
		self:SetBodygroup(1,math.random(0,0)) -- Pauldron
		self:SetBodygroup(3,math.random(0,18)) -- Head
		self:SetBodygroup(4,math.random(0,2)) -- Hats
		self:SetBodygroup(5,math.random(0,0)) -- Pouches
		self:SetBodygroup(6,math.random(0,0)) -- Webbings
		self:SetBodygroup(7,math.random(0,0)) -- Backpacks
