...
function MQS.QuestStatusCheck(ply, tk)
	if ply.MQSdata.Stored and ply.MQSdata.Stored.QuestList and ply.MQSdata.Stored.QuestList[tk] then return ply.MQSdata.Stored.QuestList[tk] end
	return false
end
...
function MQS.CanStartTask(tk, ply, npc, force)
	...
	if not force then
		...
		if MQS.TaskQueue[tk] and MQS.TaskQueue[tk] > CurTime() then return false, MSD.GetPhrase("q_time_wait") end
