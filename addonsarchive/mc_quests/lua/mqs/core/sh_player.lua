...
function MQS.IsAdministrator(ply)
    return MQS.Administrators[ply:GetUserGroup()]
end

function MQS.IsEditor(ply)
    return MQS.Editors[ply:GetUserGroup()] or MQS.IsAdministrator(ply)
end
...
function MQS.HasQuest(ply)
	...
	return MQS.GetNWdata(ply, "active_quest")
end
...
if CLIENT then
	...
	net.Receive("MQS.SetPData", function()
		...
		if ply and IsValid(ply) and ply:IsPlayer() then
			...
			if not data[2] then
				ply.MQSdata[data[1]] = nil
			else
				ply.MQSdata[data[1]] = data[2]
			end
		else
			...
			if not data[2] then
				LocalPlayer().MQSdata_self[data[1]] = nil
			else
				LocalPlayer().MQSdata_self[data[1]] = data[2]
			end
		end
	end)
