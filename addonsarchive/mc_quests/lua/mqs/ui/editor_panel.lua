...
local function LoadAutoSave()
	if file.Exists("mqs/autosave.txt", "DATA") then
		...
		active_quest = util.JSONToTable(jsontable)
	end
end
...
function MQS.OpenAdminMenu(parrent)
	if MQS.SetupMenu then
		if parrent then
			MQS.SetupMenu:Center()
		elseif not MQS.SetupMenu:IsVisible() then
			MQS.SetupMenu:AlphaTo(255, 0.4)
			MQS.SetupMenu:Show()
			MQS.SetupMenu:Center()
		else
	...
	panel.Paint = function(self, w, h)
		...
		draw.DrawText(string.upper(Ln("admin_menu")), "MSDFont.25", 12, 12, color_white, TEXT_ALIGN_LEFT)
	end
	...
	panel.Canvas:SetSize(panel:GetWide() - 252, panel:GetTall() - 52)
	panel.Canvas:SetPos(252, 52)
	...
	if MQS.IsAdministrator(LocalPlayer()) then
		...
		-- buttons[4] = {
		-- 	Ln("quest_tools"), MSD.Icons48.tools, function()
		-- 		openPage("quest_tools", true)
		-- 	end
		-- }
	end

	panel.Menu = vgui.Create("MSDPanelList", panel)
	panel.Menu:SetSize(250, panel:GetTall() - 52)
	panel.Menu:SetPos(0, 52)
	...
	local function PopupMenu(text, x, y, w, dwd)
		local pl, child = MSD.WorkSpacePanel(panel, text, x, y, false)
	...
	pages["edit"] = function(open, id, quest)
		if open then
			if not quest and id == true then
				...
				if file.Exists("mqs/autosave.txt", "DATA") and file.Open("mqs/autosave.txt", "r", "DATA"):Size() > 71 then
	...
	pages["edit_page1"] = function()
		...
		MSD.TextEntry(panel.Canvas, "static", nil, 2, 50, Ln("e_blank_dis"), Ln("q_dotime") .. "(" .. Ln("in_sec") .. "):", active_quest.do_time, function(self, value)
		...
		MSD.DTextSlider(panel.Canvas, "static", nil, 2, 50, Ln("q_dotime_ok"), Ln("q_dotime_fail"), active_quest.reward_on_time, function(self, value)
		...
		MSD.BoolSlider(panel.Canvas, "static", nil, 3, 50, Ln("q_death_fail"), active_quest.fail_ondeath, function(self, value)
			active_quest.fail_ondeath = value
		end)

		MSD.TextEntry(panel.Canvas, "static", nil, 3, 50, Ln("e_blank_dis"), Ln("cooldown_ok") .. "(" .. Ln("in_sec") .. "):", active_quest.cool_down, function(self, value)
			active_quest.cool_down = tonumber(value) or nil
		end, true, nil, nil, true)

		MSD.TextEntry(panel.Canvas, "static", nil, 3, 50, Ln("e_blank_dis"), Ln("cooldown_fail") .. "(" .. Ln("in_sec") .. "):", active_quest.cool_down_onfail, function(self, value)
		...
		if MQS.Config.NPC.enable then
			...
			if MCS then
				for k, v in pairs(MCS.Spawns) do
					if istable(active_quest.lin) and active_quest.link.id == k then
		...
		MSD.Button(panel.Canvas, "static", nil, 1, 50, Ln("s_quest_blacklist"), function()
			local _, child = MSD.WorkSpacePanel(panel, Ln("s_quest_blacklist_desc"), 1, 1.2, false)
	...
	pages["edit_page2"] = function()
		...
		for t_id, object in pairs(active_quest.objects) do
			if not MSD.ObjeciveList[object.type] then
				MSD.BigButton(panel.Canvas, "static", nil, 4, 120, Ln("q_incvobj"), MSD.Icons48.alert, function() end, MSD.Config.MainColor["r"])
				continue
			end

				MSD.BigButton(panel.Canvas, "static", nil, 4, 120, object.type, MSD.ObjeciveList[object.type].icon, function()
				if SwapEvents then
			...
			end, object.desc ~= object.type and object.desc or "")
		end

		MSD.BigButton(panel.Canvas, "static", nil, 4, 120, Ln("q_newobj"), MSD.Icons48.plus, function(self)
			if (IsValid(self.Menu)) then
				self.Menu:Remove()
				self.Menu = nil
			end
	...
	pages["edit_objective"] = function(t_id, object)
		...
		obj_sets.Populate = function()
			obj_sets:Clear()
			MSD.ObjeciveList[object.type].builUI(t_id, object, obj_sets, PopupMenu)
		end
		...
		obj_events.Populate = function()
			...
			MSD.Button(obj_events, "static", nil, 1, 50, Ln("q_eventadd"), function(self)
				...
				self.Menu = MSD.MenuOpen(false, self, true)
	...
	pages["npcs"] = function()
		...
		local function EditNPC(id, npc)
			...
			local vecd = MSD.VectorDisplay(rwd_list, "static", nil, 1, 50, Ln("spawn_point"), npc.spawns[map][1])
			local amgl = MSD.AngleDisplay(rwd_list, "static", nil, 1, 50, Ln("spawn_ang"), npc.spawns[map][2])
	...
	pages["settings"] = function()
		...
		rwd_list.Populate = function()
			...
			MSD.Button(rwd_list, "static", nil, 1, 50, Ln("set_ui"), function()
				rwd_set.Populate("ui")
			end)
		...
		rwd_set.SetingList["ui"] = function()
			MSD.Header(rwd_set, Ln("set_ui"))
			local combo = MSD.ComboBox(rwd_set, "static", nil, 1, 50, "Language:", Ln("none"))
			...
			MSD.DTextSlider(rwd_set, "static", nil, 1, 50, Ln("set_ui_blur"), Ln("set_ui_mono"), MQS.Config.UI.Blur, function(self, value)
				MQS.Config.UI.Blur = value
				sld1.disabled = value
			end)

			MSD.BoolSlider(rwd_set, "static", nil, 1, 50, Ln("set_ui_vignette"), MQS.Config.UI.Vignette, function(self, value)
				MQS.Config.UI.Vignette = value
			end)

			sld1 = MSD.VolumeSlider(rwd_set, "static", nil, 1, 50, Ln("set_ui_brightness"), MQS.Config.UI.BgrColor.r / 255, function(self, var)
				local c = math.Clamp(math.Round(var * 255), 30, 250)
				MQS.Config.UI.BgrColor = Color(c, c, c)
			end)

			sld1.disabled = MQS.Config.UI.Blur

			MSD.ColorSelector(rwd_set, "static", nil, 1, 50, Ln("set_ui_color"), MSD.Config.MainColor.p, function(self, color)
	...
	-- pages["quest_tools"] = function()
	-- 	MSD.Header(panel.Canvas, Ln("quest_tools"))
