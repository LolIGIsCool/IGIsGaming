...
function ENT:ReloadWeapon()
	...
	self:SetAmmoDetonators( self:GetMaxAmmoDetonators() )

	self:OnReloadWeapon()
end

function ENT:TakeDetonatorsAmmo(amount)
	amount = amount or 1
	self:SetAmmoDetonators(math.max(self:GetAmmoDetonators() - amount, 0))
end

function ENT:CanDropDetonators()
	return self.NextDropDetonators <= CurTime()
end

function ENT:SetNextDropDetonators(amount)
	self.NextDropDetonators = CurTime() + amount
end

function ENT:DropDetonators()
	if not self:CanDropDetonators() then return end
	if self:GetAmmoDetonators() < 1 then return end
	self:SetNextDropDetonators(0.5)

	local pos = self:GetBonePosition(self:LookupBone("Rear_Bomb_Hatch"))
	local ent = ents.Create("laat_detonator")
		ent:SetPos(pos)
		ent:SetAngles(self:GetAngles())
		ent:Spawn()
		ent:Activate()
		ent.Attacker = self:GetDriver()
		ent.Inflictor = self
		ent:SetVelocity(self:GetVelocity())

	constraint.NoCollide(ent, self, 0, 0)

	self:TakeDetonatorsAmmo()
end
...
function ENT:GunnerWeapons( Driver, Pod )
	...
	if Driver:KeyDown(IN_JUMP) && Driver:KeyDown(IN_ZOOM) then
		if self.NextUseSpotlight < CurTime() then
			self.NextUseSpotlight = CurTime() + 1 
			self:SetSpotlightOn(not self:GetSpotlightOn())
		end
