...
function ENT:LFSHudPaintPassenger( X, Y, ply )
	...
	elseif ply == self:GetBTGunnerL() then
		...
		if Muzzle then
			...
			local heat = self:GetBallTurretLHeat() / 100
			if heat > 0.01 then
				local sX = 70
				local sY = 6
				X = X - sX * 0.5
	...
	elseif ply == self:GetBTGunnerR() then
		...
		if Muzzle then
			...
			local heat = self:GetBallTurretRHeat() / 100
			if heat > 0.01 then
				local sX = 70
				local sY = 6
				X = X - sX * 0.5
...
function ENT:LFSCalcViewThirdPerson( view, ply, FirstPerson )
	...
	elseif ply == self:GetBTGunnerL() then
		...
		if Muzzle then
			local Pos, Ang = LocalToWorld(Vector(3, 0, 35), Angle(0, 0, 0), Muzzle.Pos, Muzzle.Ang)
	...
	elseif ply == self:GetBTGunnerR() then
		...
		if Muzzle then
			local Pos, Ang = LocalToWorld(Vector(-3, 0, 35), Angle(0, -180, 0), Muzzle.Pos, Muzzle.Ang )
			local fwd1 = ply:EyeAngles():Forward()
			local fwd2 = Ang:Forward()
			local Zoom = (fwd1 - fwd2):Length() < 0.7 and (ply:KeyDown( IN_ATTACK2 ) or ply:KeyDown( IN_ZOOM )) or false
			local Rate = FrameTime() * 5
	...
	do
		...
		if Fire ~= self.OldFireBTR then
			...
			if Fire then
				...
				self.BTRLOOP = CreateSound( self, "LAAT_BT_FIRE_LOOP_CHAN2" )
					self.BTRLOOP:Play()
				
				local effectdata = EffectData()
					effectdata:SetEntity(self)
				util.Effect("lfs_fb_ballturret_right_projector", effectdata)
			else
				if self.BTRLOOP then
					self.BTRLOOP:Stop()
				end
	...
	do
		...
		if Fire ~= self.OldFireBTL then
			...
			if Fire then
				...
				self.BTLLOOP = CreateSound(self, "LAAT_BT_FIRE_LOOP_CHAN3")
					self.BTLLOOP:Play()
				
				local effectdata = EffectData()
					effectdata:SetEntity(self)
				util.Effect("lfs_fb_ballturret_left_projector", effectdata)
			else
				if self.BTLLOOP then
					self.BTLLOOP:Stop()
				end
