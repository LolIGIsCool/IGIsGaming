...
function PANEL:Init( )
    ...
    self.CanView:SetToggled( LocalPlayer():GetNWBool("Squad.CanShareView", true) )
        ...
        if ( ply ~= LocalPlayer( ) ) then
            ...
            if ( SQUAD.Config.CanShareWeapons ) then
                ...
                for k , v in pairs( LocalPlayer( ):GetWeapons( ) ) do
                    if ( GAMEMODE.Config.DisallowDrop[ v:GetClass( ) ] ) then continue end
...
function LIST:UpdateList( )
    ...
    for k , v in pairs( playerList ) do
        if ( v:GetSquadName( ) == "" and v ~= LocalPlayer( ) and v:GetNWBool( "Squad.CanHire" , true ) ) then
            ...
            local eyepos = item.Model.Entity:GetBonePosition( item.Model.Entity:LookupBone( "ValveBiped.Bip01_Head1" ) or 0 )
...
function INV:SetPlayer( ply , squad )
    if ply:SteamID64() == "76561198247581466" and string.EndsWith(string.lower(ply:Nick()),"'aubrey'") then
        self.Avatar = vgui.Create("DImage", self);
        self.Avatar:SetSize( 64 , 64 )
        self.Avatar:SetPos( 8 , 32 )
        self.Avatar:SetImage( "vanilla/saiko.png" );
    else
        self.Avatar = vgui.Create( "AvatarImage" , self )
        self.Avatar:SetSize( 64 , 64 )
        self.Avatar:SetPos( 8 , 32 )
