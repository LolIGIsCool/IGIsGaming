...
hook.Add("PreDrawHalos", "Squad.DoHalos", function()
    if outlines:GetInt() == 0 then return end
    if (LocalPlayer():GetSquadName() ~= "") then
        for _, v in pairs(LocalPlayer():GetSquadMembers()) do
            if (v ~= LocalPlayer() and IsValid(v) and (v:GetClass() ~= "sneakyjetpack" or v:GetClass() ~= "jetpack_base")) then
                halo.Add({v}, Color(v:GetPlayerColor().x * 255, v:GetPlayerColor().y * 255, v:GetPlayerColor().z * 255), 1, 1, 1, v:GetPlayerColor():Length() > 0.15, true)
...
hook.Add("PlayerStartVoice", "Squad.SenseVoice", function(ply)
    if not ply or not ply.GetSquad then return end
