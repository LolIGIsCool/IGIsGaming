...
function SQUAD:CreateGroup( tag , name , creator )
...
function SQUAD:AskJoin( tag , ply )
...
function SQUAD.PlyMeta:RemoveFromSquad( )
    ...
    if ( #SQUAD.Groups[ tag ].Members == 0 ) then
        ...
        if ( SQUAD.Groups[ tag ].Creator == self ) then
            local newLeader = table.Random( SQUAD.Groups[ tag ].Members )
...
net.Receive( "Squad.SendInvitation" , function( _ , ply )
    local ent = net.ReadEntity( )
    if ((ply.SquadInvitationCooldown or 0) > CurTime()) then return end
    ply.SquadInvitationCooldown = CurTime() + 3
...
hook.Add( "IGPlayerSay" , "Squad.ChatSystem" , function( ply , text )
