...
ENT.PrintName = "Walker2 Test Base"
ENT.Author = "Liam0102"
--- Edits by Cody, purely for testing, full credit goes to Liam0102 ---
--- TEMPORARY ---
...
if SERVER then
function WALKER2CreateBulletStructure(dmg,color,nosplashdamage)
...
function ENT:Walker2Classing()
	local mod = 1;
	if(self.Walker2Class == 2) or (self.Walker2Class == 3) then
		//Probes
...
function ENT:Enter(p,driver)
	...
	p:SetNWInt("SW_Walker2_MaxSpeed",self.BoostSpeed)
	self:Walker2Classing();
...
function ENT:Think()
	...
	if(self.Inflight) then
		...
		if(IsValid(self.Pilot)) then
			...
			if(self.Pilot:KeyDown(IN_SPEED) and self.Walker2Class == 1) then
				self:Boost();
			end
...
hook.Add("PlayerEnteredVehicle","SWWalker2Use", function(p, e)
	if(IsValid(e) and IsValid(p) and p:IsPlayer()) then
		local walker2 = e:GetParent();
		if(IsValid(walker2) and walker2.IsSWVehicle) then
			if(e.IsWalker2Chair) then
				if(!IsValid(walker2.Pilot) and !walker2.Inflight) then
					walker2:Enter(p,true);
				end
			elseif(e.Walker2Passenger) then
				if(!IsValid(walker2.Passenger)) then
					walker2:Enter(p,false);
				end
			end
		end
	end
			...
			*/
			...
			if(self.EngineOn) then
				...
				elseif(self.Pilot:KeyDown(IN_FORWARD)) then
					self.num = self.num + self.ForwardSpeed/10;
				elseif(self.Pilot:KeyDown(IN_BACK)) then
					self.num = self.num - self.ForwardSpeed/10;
				elseif(self.Pilot:KeyDown(IN_FORWARD) and self.Pilot:KeyDown(IN_SPEED) and self.Walker2Class == 2) then
				...
				if(!self.Boosting) then
					...
					if(self.Pilot:KeyDown(IN_SPEED) and self.Walker2Class == 2) then
						max = self.BoostSpeed;
					end
				...
				if(IsValid(self.Pilot)) then
					self.Pilot:SetNWInt("SW_Walker2_Speed",self.Accel.FWD);
				end
				
				if(self.Walker2Class == 3) then
				    if(self.Pilot:KeyDown(IN_MOVERIGHT)) then
					    self.YawAccel = -3 - (self.Accel.FWD/100);
				...
				elseif(self.Walker2Class == 2) then
				    if(self.Pilot:KeyDown(IN_MOVERIGHT)) then
					    self.YawAccel = -30 - (self.Accel.FWD/100);
				    elseif(self.Pilot:KeyDown(IN_MOVELEFT)) then
					    self.YawAccel = 30 + (self.Accel.FWD/100);
				    else
					    self.YawAccel = 0;
...
function SWWalker2StandbyPickup(p,e)
	if(IsValid(e) and e.IsSWVehicle) then
		e.ShouldStandby = false;
	end
end
hook.Add( "PhysgunPickup", "SWWalker2StandbyPickup", SWWalker2StandbyPickup )

function SWWalker2StandbyDrop(p,e)
	if(IsValid(e) and e.IsSWVehicle) then
		e.ShouldStandby = true;
	end
end
hook.Add("PhysgunDrop", "SWWalker2StandbyDrop", SWWalker2StandbyDrop);
...
if CLIENT then
	...
	function ENT:Initialize()
		...
		self.SoundsOn = {}
		if (self.Sounds and self.Sounds.Walk) then
			self.WalkSound = self.WalkSound or CreateSound(self.Entity,self.Sounds.Walk);
		end
	...
	function ENT:StartClientsideSound(mode)
		if(not self.SoundsOn[mode]) then
			if(mode == "Walk" and self.WalkSound) then
				self.WalkSound:Stop();
				self.WalkSound:SetSoundLevel(110);
				self.WalkSound:PlayEx(100,100);
			end
	...
	function ENT:Think()
		...
		local p = LocalPlayer();
		local Walker2Active = self:GetNWBool("Flying" ..self.Vehicle);
		
		if self:GetNWInt("Speed") > 0 and (Walker2Active) or ((p:KeyDown(IN_MOVERIGHT) or p:KeyDown(IN_MOVELEFT)) and (Walker2Active)) then
			local IsDriver = p:GetNWEntity(self.Vehicle) == self.Entity;
			self:StartClientsideSound("Walk");
			...
			Speed = p:GetNWInt("SW_Walker2_Speed") or self:GetNWInt("Speed");
			MaxSpeed = p:GetNWInt("SW_Walker2_MaxSpeed") or self:GetNWInt("MaxSpeed");
			
			if(self.SoundsOn.Walk) then
				...
				self.WalkSound:ChangePitch(math.Clamp(60 + pitch/25,75,100) + doppler,0);
			end
		
		    Health = self:GetNWInt("Health");
			
		else
			self:StopClientsideSound("Walk");
		end
	end
	...
	function SW_Walker2_DrawHull(StartHealth)
	...
	function SW_Walker2_DrawSpeedometer()
	...
	function SW_Walker2_Reticles(self,WeaponsPos) 
		local p = LocalPlayer();
		...
		for i=1,table.Count(WeaponsPos) do
			...
			if(IsValid(tr.Entity)) then
				if(IsSWWalker2(tr.Entity:GetClass()) and tr.Entity != self) then
