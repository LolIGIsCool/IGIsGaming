...
local server_files = {}
net.Receive("StonemanNPC_SyncToClient", function()
	MCS.Spawns = net.ReadTable()
	server_files = net.ReadTable()
end)

MCS.PanelControll.FileOpenServer = function(parent, func)
	local mx, my = gui.MousePos()
	local frame_server = vgui.Create("DFrame")
	frame_server:SetSize(300, 400)
	frame_server:SetPos(mx, my)
	frame_server:MakePopup()
	frame_server:SetTitle("File List")
	frame_server.StartT = CurTime() + 2

	frame_server.Think = function(self)
		if not IsValid(parent) or (not self:HasFocus() and self.StartT < CurTime()) then
			self:Close()
		end
	end

	local FileListServer = vgui.Create("DListView", frame_server)
	FileListServer:AddColumn("File name")
	FileListServer:Dock(FILL)
	FileListServer:SetMultiSelect(false)
	FileListServer:SetHideHeaders(false)
	for k, v in pairs(server_files) do
		local line = FileListServer:AddLine(v)

		line.OnSelect = function()
			net.Start("StonemanNPC_GetServerTable")
			net.WriteString(v)
			net.SendToServer()

			net.Receive("StonemanNPC_SendBack", function()
				local newjsontable = net.ReadString()
				localnpc = util.JSONToTable(newjsontable) 
				frame_server:Close()
				func()
			end)
		end
...
MCS.PanelControll.FileOpenServerDelete = function(parent, func)
	local mx, my = gui.MousePos()
	local frame_server_del = vgui.Create("DFrame")
	frame_server_del:SetSize(300, 400)
	frame_server_del:SetPos(mx, my)
	frame_server_del:MakePopup()
	frame_server_del:SetTitle("File List")
	frame_server_del.StartT = CurTime() + 2

	frame_server_del.Think = function(self)
		if not IsValid(parent) or (not self:HasFocus() and self.StartT < CurTime()) then
			self:Close()
		end
	end

	local FileListServerDelete = vgui.Create("DListView", frame_server_del)
	FileListServerDelete:AddColumn("File name")
	FileListServerDelete:Dock(FILL)
	FileListServerDelete:SetMultiSelect(false)
	FileListServerDelete:SetHideHeaders(false)
	for k, v in pairs(server_files) do
		local line = FileListServerDelete:AddLine(v)

		line.OnSelect = function()
			net.Start("StonemanNPC_DeleteServerTable")
			net.WriteString(v)
			net.SendToServer()
			frame_server_del:Close()
			func()
		end
	end

	return frame_server_del
end
...
function MSD.OpenAdminMenu(panel, mainPanel)
	...
	pages["final"] = function()
		...
		MSD.BigButton(panel.Canvas, "static", nil, 2, 120, "Respawn all NPCs", MSD.Icons48.account_convert, function()
			if not MQS.Config.Administrators[LocalPlayer():GetUserGroup()] then return end
			surface.PlaySound("mqs/done/1.mp3")
			net.Start("MCS.StartNPCRespawn")
			net.SendToServer()
		end)

		MSD.Header(panel.Canvas, "Server Options")

		MSD.BigButton(panel.Canvas, "static", nil, 2, 120, "Save to server", MSD.Icons48.save, function()
			surface.PlaySound("mqs/done/4.mp3")

			net.Start("StonemanNPC_SendToServer")
				net.WriteTable(localnpc)
			net.SendToServer()
		end)

		MSD.BigButton(panel.Canvas, "static", nil, 2, 120, "Open & Edit Server Files", MSD.Icons48.folder_open, function()
			if not IsValid(oframe) then
				oframe = MCS.PanelControll.FileOpenServer(MainM, function()
					surface.PlaySound("mqs/done/1.mp3")
				end)
			end
		end)

		MSD.BigButton(panel.Canvas, "static", nil, 2, 120, "Delete Server Files", MSD.Icons48.folder_open, function()
			if not IsValid(oframe) then
				oframe = MCS.PanelControll.FileOpenServerDelete(MainM, function()
					surface.PlaySound("mqs/done/1.mp3")
				end)
			end
		end)

		MSD.Header(panel.Canvas, "Get Help")
