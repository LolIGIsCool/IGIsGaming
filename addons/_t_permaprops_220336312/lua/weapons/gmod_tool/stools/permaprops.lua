...
function TOOL:RightClick(trace)
	...
	ulx.logString( string.format( "[PERMAPROPS] %s erased %s with a model of %s from the database", ply:Nick(), ent:GetClass(), ent:GetModel() ) )
...
function TOOL:Reload(trace)
	...
	local ent = trace.Entity
	local ply = self:GetOwner()  
	if !ply:IsAdmin() then ply:ChatPrint("You don't have permissions to do that!") return end

	if not PermaProps then ply:ChatPrint( "ERROR: Lib not found" ) return end

	if (not trace.Entity:IsValid()) then 
		if not ply.reloadcountpp then ply.reloadcountpp = 0 end
		ply.reloadcountpp = ply.reloadcountpp + 1
		if ply.reloadcountpp <= 1 then
			ply:ChatPrint("Anti-Crasher: Reload two more times to confirm perma prop reload")
		elseif ply.reloadcountpp <= 2 then
			ply:ChatPrint("Anti-Crasher: Reload one more time to confirm perma prop reload")
		elseif ply.reloadcountpp >= 3 then
			ply:ChatPrint("Anti-Crasher: Reload confirmed, reloading perma props")
			ply.reloadcountpp = 0
