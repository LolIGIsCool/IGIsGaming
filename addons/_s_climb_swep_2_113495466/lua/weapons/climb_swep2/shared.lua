...
function SWEP:Think()
    ...
    if self.Jumps != self.Owner:GetNWInt("ClimbJumps") then self.Owner:SetNWInt("ClimbJumps", self.Jumps) hook.Call("playerclimbjump",GAMEMODE,self.Owner,self.Jumps) end
    ...
    elseif self.Owner:KeyDown(IN_FORWARD) then
        local maxjumps = 0
        if _G.HasAugment(self.Owner, "The Mountaineer") then maxjumps = GetConVarNumber("climbswep2_maxjumps") + 1 else maxjumps = GetConVarNumber("climbswep2_maxjumps") end
        if self.Owner:GetRegiment() == "501st Scout" then maxjumps = maxjumps + 2 end
		if self.Owner:GetRegiment() == "Imperial Commandos" then maxjumps = maxjumps + 2 end
		if self.Owner:GetRegiment() == "Shadow Guard" then maxjumps = maxjumps + 2 end
		if self.Owner:GetRegiment() == "Purge Trooper" then maxjumps = maxjumps + 2 end
		if self.Owner:GetRegiment() == "439th ARC Trooper" then maxjumps = maxjumps + 1 end
        if self.Owner:KeyDown(IN_USE) and !self.Owner:OnGround() and self.CanWallRun and self.Jumps < maxjumps and !self.Grab then

            local traceData = {};
            if !traceData then return end
...
-- We'll have to be off the ground to start climbing!
    ...
    if self.Owner:GetNWBool("ClimbWallJump") then
        ...
        local jumpPower = 300;
        if _G.HasAugment(self.Owner, "Tauntaun Legs I") then jumpPower = jumpPower + (jumpPower * 0.5) end
        if _G.HasAugment(self.Owner, "Tauntaun Legs II") then jumpPower = jumpPower + (jumpPower * 0.5) end
        if _G.HasAugment(self.Owner, "Tauntaun Legs I") then _G.ActivateAugment(self.Owner, "Tauntaun Legs I", 1) end
    ...
    local maxjumps = 3
    if _G.HasAugment(self.Owner, "The Mountaineer") then maxjumps = maxjumps + 1 else maxjumps = maxjumps end
    if self.Owner:GetRegiment() == "501st Scout" then maxjumps = maxjumps + 2 end
	if self.Owner:GetRegiment() == "Imperial Commandos" then maxjumps = maxjumps + 2 end
	if self.Owner:GetRegiment() == "Shadow Guard" then maxjumps = maxjumps + 2 end
	if self.Owner:GetRegiment() == "Purge Trooper" then maxjumps = maxjumps + 2 end
	if self.Owner:GetRegiment() == "439th ARC Trooper" then maxjumps = maxjumps + 1 end
    if ( (self.Jumps == 0 and trace.HitPos:Distance(ShootPos) > 500) or self.Jumps > (maxjumps - 1) or trace.HitSky) then return false end
...
function SWEP:SecondaryAttack()
    ...
    local ThinkFunction = function()
        ...
        if (Ply) and Ply:IsValid() and Ply:IsPlayer() and !Ply:Alive() then hook.Remove("Think", "ClimbGrab"..Ply:UniqueID())
...
function SWEP:OnRemove()
    if SERVER then
        hook.Remove("Think", "ClimbGrab"..self.Owner:UniqueID())
    end
end
...
function SWEP:DrawHUD()
    ...
    */
    local MaxJumps = GetConVarNumber("climbswep2_maxjumps")
    if self.Owner:GetNWBool("Mountaineer",false) then MaxJumps = MaxJumps + 1 end
    if self.Owner:GetRegiment() == "501st Scout" then MaxJumps = MaxJumps + 2 end
	if self.Owner:GetRegiment() == "Imperial Commandos" then MaxJumps = MaxJumps + 2 end
	if self.Owner:GetRegiment() == "Shadow Guard" then MaxJumps = MaxJumps + 2 end
	if self.Owner:GetRegiment() == "Purge Trooper" then MaxJumps = MaxJumps + 2 end
	if self.Owner:GetRegiment() == "439th ARC Trooper" then MaxJumps = MaxJumps + 1 end
    local Jumps = LocalPlayer():GetNWInt("ClimbJumps")
    ...
    // Draw Jump-Monitor
    --_G.vanillaBlurPanel(ScrW() / 2 - Width / 2, ScrH() - Height * 2, Width, Height, Color(0, 0, 0, 100))
    --if (MaxJumps - Jumps) > 0 then _G.vanillaBlurPanel( ScrW() / 2 - Width / 2, ScrH() - Height * 2, Width * (MaxJumps - Jumps) / MaxJumps, Height, defconcolors[tonumber(dc[1])]) end
    --draw.SimpleTextOutlined("Jumps: "..(MaxJumps - Jumps).." of "..MaxJumps, "Default", ScrW() / 2, ScrH() - 33, Color(255, 255, 255, 255), TEXT_ALIGN_CENTER, TEXT_ALIGN_TOP, 1, Color(0,0,0,255));
    local iconsize, padding, border = 22, 12, 8
    local wideness = iconsize * MaxJumps + padding * (MaxJumps - 1)

    -- pulsating light
    local fuckery = math.sin((CurTime() * 4) % 450)^2 * 225 + 30

    if LocalPlayer():GetNWBool("ClimbFalling") then
		_G.vanillaBlurPanel(ScrW() / 2 - wideness / 2 - border, ScrH() - iconsize * 2 - border, wideness + border * 2, iconsize + border * 2, Color(203, 64, 64, fuckery))
	else
		_G.vanillaBlurPanel(ScrW() / 2 - wideness / 2 - border, ScrH() - iconsize * 2 - border, wideness + border * 2, iconsize + border * 2, Color(0, 0, 0, 100))

        for i=1,MaxJumps do
            --_G.vanillaBlurPanel((iconsize * (i - 1) + padding * (i - 1)) + ScrW() / 2 - wideness / 2, ScrH() - iconsize * 2, iconsize, iconsize, Color(0, 0, 0, 100))
            local culla
            if (MaxJumps - Jumps) >= i then
                --surface.SetDrawColor(25, 255, 125, 255)
                --surface.SetDrawColor(255, 255, 255, 255)
                culla = defconcolors[tonumber(dc[1])];
            elseif LocalPlayer():GetNWBool("ClimbWallJump") then
                --surface.SetDrawColor(51, 181, 229, fuckery)
                culla = Color(51, 181, 229, fuckery)
            else
                surface.SetDrawColor(255,255,255,1)
                culla = Color(20,20,20,80)
            end
                --surface.SetMaterial(Material("vanilla/skillicon/tauntaunlegs1.png"))
                --surface.DrawTexturedRect((iconsize * (i - 1) + padding * (i - 1)) + ScrW() / 2 - wideness / 2, ScrH() - iconsize * 2, iconsize, iconsize)
                --_G.vanillaBlurPanel((iconsize * (i - 1) + padding * (i - 1)) + ScrW() / 2 - wideness / 2, ScrH() - iconsize * 2, iconsize, iconsize, culla)
                //surface.DrawCircle((iconsize * (i - 1) + (iconsize / 2) + padding * (i - 1)) + ScrW() / 2 - wideness / 2, ScrH() + (iconsize / 2) - iconsize * 2, iconsize / 2, culla.r, culla.g, culla.b, culla.a )

                surface.SetDrawColor(culla);
                draw.NoTexture();
                Circle((iconsize * (i - 1) + (iconsize / 2) + padding * (i - 1)) + ScrW() / 2 - wideness / 2, ScrH() + (iconsize / 2) - iconsize * 2, iconsize / 2, 20);
            --end
        end
    end
