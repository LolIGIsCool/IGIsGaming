...
function ENT:AIGetTarget()
	...
	self.NextAICheck = CurTime() + 0.5

	local players = player.GetAll()

	local ClosestTarget = NULL
	local TargetDistance = 4000
	...
	if not simfphys.LFS.IgnorePlayers then
		for k, v in pairs( ents.GetAll() ) do
			if IsValid( v ) then
				if v == self:GetVar("dest") then
					local Dist = (v:GetPos() - MyPos):Length()
					if Dist <= TargetDistance then
						self.InRangeForSpawn = true
					else
						self.InRangeForSpawn = false
					end
					if 1 == 1 then
