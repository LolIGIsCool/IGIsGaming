...
function ENT:CustomOnInitialize() --[[Imperial Navy Trooper Bodygroups]]--
	if math.random(1,0) == 1 then
		...
		self:SetBodygroup(1,math.random(1,1)) -- Headwear
		self:SetBodygroup(2,math.random(0,1)) -- Uniform
		self:SetBodygroup(3,math.random(0,1)) -- Code Cylinder 1
		self:SetBodygroup(4,math.random(0,0)) -- Code Cylinder 2
		self:SetBodygroup(5,math.random(0,1)) -- Code Cylinder 3
		self:SetBodygroup(6,math.random(0,0)) -- Code Cylinder 4
		self:SetBodygroup(7,math.random(1,6)) -- Belt Boxes
		self:SetBodygroup(8,math.random(0,0)) -- Gunner Bib
		self:SetBodygroup(9,math.random(0,3)) -- Holster
	end
end
...
ENT.ItemDropsOnDeathChance = 1.5 -- If set to 1, it will always drop it
...
ENT.SkillPreset = "waffenoff" --Can be heerprivate heerofficer waffeninf waffenoff
ENT.SkillSet = 2
--Class
...
ENT.Commanding_maxsoldier = 20 --This should change per commander (6 for Sergeants, 15 for Officers, 100 for Generals)
