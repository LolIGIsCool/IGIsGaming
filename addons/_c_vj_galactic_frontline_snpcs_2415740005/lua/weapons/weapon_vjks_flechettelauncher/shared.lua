...
SWEP.PrintName					= "Imperial Flechette Launcher"
SWEP.Author 					= "DrVrej"
SWEP.Contact					= "http://steamcommunity.com/groups/vrejgaming"
SWEP.Purpose					= "Launches a bundle of flechettes at targets. Mainly designed for riot control."
...
SWEP.UseHands					= true
	...
	-- NPC Settings ---------------------------------------------------------------------------------------------------------------------------------------------
SWEP.NPC_NextPrimaryFire 		= 1 -- Next time it can use primary fires
...
SWEP.NPC_FiringDistanceScale = 5.5 -- Changes how far the NPC can fire | 1 = No change, x < 1 = closer, x > 1 = farther
...
SWEP.NPC_ReloadSoundLevel = 60 -- How far does the sound go?
	-- Main Settings ---------------------------------------------------------------------------------------------------------------------------------------------
SWEP.WorldModel					= "models/krieg/galacticempire/weapons/blastech_c20.mdl" 
...
function SWEP:CustomOnPrimaryAttack_BeforeShoot()
	...
	local proj = ents.Create("obj_vjks_flechette_detonator")
	...
	if IsValid(phys) then
		if self:GetOwner():IsPlayer() then
			phys:SetVelocity(self:GetOwner():GetAimVector() * 400)
		else
			phys:SetVelocity(self:GetOwner():CalculateProjectile("Line", self:GetNWVector("VJ_CurBulletPos"), self:GetOwner():GetEnemy():GetPos() + self:GetOwner():GetEnemy():OBBCenter(), 177050))
		end
	end
end
