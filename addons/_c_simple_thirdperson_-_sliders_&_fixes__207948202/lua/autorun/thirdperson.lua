...
if CLIENT then	
	...
	hook.Add("CalcView","SimpleTP.Camera.View",function(ply, pos, angles, fov)
		...
		--if isEnabled and IsValid(ply) and ply:GetActiveWeapon():GetClass() ~= "weapon_lightsaber" then
		
		if isEnabled and IsValid(ply) then
			if ply:HasWeapon("weapon_lightsaber") and ply:GetActiveWeapon():GetClass() == "weapon_lightsaber" then 
				return 
			end
		
			if ply.FSpectating then
				--return
				print("meme")
			end
