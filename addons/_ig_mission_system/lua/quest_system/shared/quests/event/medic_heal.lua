QUEST.Name = "Stayin' Alive"
QUEST.Instruction = "Heal 500 health worth of damage." -- %s will equal the amount
QUEST.Info = "Use your bacta injector to heal players."
QUEST.Reward = 75 -- credit reward
QUEST.Time = 0 -- how long, in seconds, a player has to complete the quest. 0 = infinite
QUEST.Amount = 500 -- how many actions required to complete quest
...
if (SERVER) then
    hook.Add("healfrombacta", "bactahealquest", function(player, healamount)
        if (player:IsPlayer() and player.QUEST_SYSTEM_ActiveQuest == "bactaheal") then
            player:QUEST_SYSTEM_Quest_UpdateProgress(player.QUEST_SYSTEM_QuestProgress + healamount)
