QUEST.Name = "Infantry Transportation"
QUEST.Instruction = "Drop 4 players off in your dropship." -- %s will equal the amount
QUEST.Info = "Obtain a dropship and drop 4 players off."
QUEST.Reward = 100 -- credit reward
QUEST.Time = 0 -- how long, in seconds, a player has to complete the quest. 0 = infinite
QUEST.Amount = 4 -- how many actions required to complete quest
QUEST.Penalty = 25 -- penalty reward
...
hook.Add("PlayerLeaveVehicle", "ImpDropShipSeatExit", function(p, v)
    if (IsValid(p) and IsValid(v)) then
        if (v.IsImpDropShipSeat) then
            ...
            local getdriver = e.Pilot
            if getdriver == p then return end

            if (getdriver:IsPlayer() and getdriver.QUEST_SYSTEM_ActiveQuest == "pilot_drop") then
                getdriver:QUEST_SYSTEM_Quest_UpdateProgress(getdriver.QUEST_SYSTEM_QuestProgress + 1)

                if (getdriver.QUEST_SYSTEM_QuestProgress >= QUEST_SYSTEM.Quests[getdriver.QUEST_SYSTEM_ActiveQuest]["Amount"]) then
                    getdriver:QUEST_SYSTEM_Quest_Finish()
                end
            end
        end
    end
