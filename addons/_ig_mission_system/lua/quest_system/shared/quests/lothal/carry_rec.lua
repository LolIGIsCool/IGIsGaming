QUEST.Name = "Arming the Recruits"
QUEST.Instruction = "A recruit is missing their blaster. Take this E-11 to the recruit room." -- %s will equal the amount
QUEST.Info = "Look for the blue sphere at the Recruit Room."
...
local pos = Vector(10901.629883, -2329.276855, -15231.968750)

if (SERVER) then
    timer.Create("isiteminzoneREC", 1, 0, function()
        for k, v in pairs(ents.FindInSphere(Vector(pos), 150)) do
            if (v:GetClass() == "carry_item") then
                ...
                if (playertoreward.QUEST_SYSTEM_ActiveQuest == "carry_rec") then
...
if (CLIENT) then
    hook.Add("PostDrawTranslucentRenderables", "DrawSphereRECCarry", function(bDepth, bSkybox)
        if (bSkybox) then return end
        if not (LocalPlayer().QUEST_SYSTEM_ActiveQuest == "carry_rec") then return end
