QUEST.Name = "High Command Aide"
QUEST.Instruction = "Take this box of office Supplies to Imperial High Command" -- %s will equal the amount
QUEST.Info = "Look for the blue sphere at Imperial High Command of where to take it."
QUEST.Reward = 50 -- credit reward
QUEST.Penalty = 500 -- penalty reward
QUEST.Time = 50 -- how long, in seconds, a player has to complete the quest. 0 = infinite
...
local pos = Vector(10549.039063, -1773.870239, -14335.968750)

if (SERVER) then
    timer.Create("isiteminzoneIHC", 1, 0, function()
        for k, v in pairs(ents.FindInSphere(Vector(pos), 150)) do
            if (v:GetClass() == "carry_item") then
                ...
                if (playertoreward.QUEST_SYSTEM_ActiveQuest == "carry_ihc") then
...
if (CLIENT) then
    hook.Add("PostDrawTranslucentRenderables", "DrawSphereIHCCarry", function(bDepth, bSkybox)
        if (bSkybox) then return end
        if not (LocalPlayer().QUEST_SYSTEM_ActiveQuest == "carry_ihc") then return end
