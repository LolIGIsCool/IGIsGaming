QUEST.Name = "Keeping us safe"
QUEST.Instruction = "Shock have used up all their cuffs. Take this box of cuffs to the security deck" -- %s will equal the amount
QUEST.Info = "Look for the blue sphere at the Security Deck."
QUEST.Reward = 50 -- credit reward
QUEST.Penalty = 500 -- penalty reward
QUEST.Time = 30 -- how long, in seconds, a player has to complete the quest. 0 = infinite
...
local pos = Vector(10568.344727, 353.595459, -15231.968750)

if (SERVER) then
    timer.Create("isiteminzoneSK", 1, 0, function()
        for k, v in pairs(ents.FindInSphere(Vector(pos), 150)) do
            if (v:GetClass() == "carry_item") then
                ...
                if (playertoreward.QUEST_SYSTEM_ActiveQuest == "carry_sk") then
...
if (CLIENT) then
    hook.Add("PostDrawTranslucentRenderables", "DrawSphereSKCarry", function(bDepth, bSkybox)
        if (bSkybox) then return end
        if not (LocalPlayer().QUEST_SYSTEM_ActiveQuest == "carry_sk") then return end
