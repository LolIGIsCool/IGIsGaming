QUEST.Name = "Helping me out"
QUEST.Instruction = "I need you to take this to a back alley. Don't look inside. I can't tell where but, go quick." -- %s will equal the amount
QUEST.Info = "HINT: Search the back alleys of the town for the drop off area."
...
local pos = Vector(10048.788086, -12655.990234, -15231.968750)

if (SERVER) then
    timer.Create("isiteminzoneALLEY", 1, 0, function()
        for k, v in pairs(ents.FindInSphere(Vector(pos), 150)) do
            if (v:GetClass() == "carry_item") then
                ...
                if (playertoreward.QUEST_SYSTEM_ActiveQuest == "carry_alley") then
...
if (CLIENT) then
    hook.Add("PostDrawTranslucentRenderables", "DrawSphereALLEYCarry", function(bDepth, bSkybox)
        if (bSkybox) then return end
        if not (LocalPlayer().QUEST_SYSTEM_ActiveQuest == "carry_alley") then return end
