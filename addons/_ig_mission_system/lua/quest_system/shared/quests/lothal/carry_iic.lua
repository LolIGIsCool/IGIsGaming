QUEST.Name = "Get Smart"
QUEST.Instruction = "Imperial Intelligence have run out of binder books, I happened to buy one from the town earlier. Take this to them for me." -- %s will equal the amount
QUEST.Info = "Look for the blue sphere at Imperial Intelligence for where to take it."
QUEST.Reward = 50 -- credit reward
QUEST.Penalty = 500 -- penalty reward
QUEST.Time = 60 -- how long, in seconds, a player has to complete the quest. 0 = infinite
...
local pos = Vector(9206.721680, -1081.522461, -14335.968750)

if (SERVER) then
    timer.Create("isiteminzoneIIC", 1, 0, function()
        for k, v in pairs(ents.FindInSphere(Vector(pos), 150)) do
            if (v:GetClass() == "carry_item") then
                ...
                if (playertoreward.QUEST_SYSTEM_ActiveQuest == "carry_iic") then
...
if (CLIENT) then
    hook.Add("PostDrawTranslucentRenderables", "DrawSphereIICCarry", function(bDepth, bSkybox)
        if (bSkybox) then return end
        if not (LocalPlayer().QUEST_SYSTEM_ActiveQuest == "carry_iic") then return end
