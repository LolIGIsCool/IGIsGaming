QUEST.Name = "Friend of the Bureau"
QUEST.Instruction = "I found this Brief Case, I think ISB might be interested in it can you take it to them for me." -- %s will equal the amount
QUEST.Info = "Look for the blue sphere at the Imperial Security Bureau HQ for where to take it."
QUEST.Reward = 50 -- credit reward
QUEST.Penalty = 500 -- penalty reward
QUEST.Time = 55 -- how long, in seconds, a player has to complete the quest. 0 = infinite
...
local pos = Vector(10576.285156, -209.195908, -14335.968750)

if (SERVER) then
    timer.Create("isiteminzoneISB", 1, 0, function()
        for k, v in pairs(ents.FindInSphere(Vector(pos), 150)) do
            if (v:GetClass() == "carry_item") then
                ...
                if (playertoreward.QUEST_SYSTEM_ActiveQuest == "carry_isb") then
...
if (CLIENT) then
    hook.Add("PostDrawTranslucentRenderables", "DrawSphereISBCarry", function(bDepth, bSkybox)
        if (bSkybox) then return end
        if not (LocalPlayer().QUEST_SYSTEM_ActiveQuest == "carry_isb") then return end
