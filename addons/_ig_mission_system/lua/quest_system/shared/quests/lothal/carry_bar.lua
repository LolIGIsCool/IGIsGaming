QUEST.Name = "Friend of the people"
QUEST.Instruction = "I need you to take these 'supplies' to the local bar in town for me." -- %s will equal the amount
QUEST.Info = "Look for the blue sphere at the in the bar in the town."
...
local pos = Vector(9656.455078, -11241.666016, -15231.968750)

if (SERVER) then
    timer.Create("isiteminzoneBAR", 1, 0, function()
        for k, v in pairs(ents.FindInSphere(Vector(pos), 150)) do
            if (v:GetClass() == "carry_item") then
                ...
                if (playertoreward.QUEST_SYSTEM_ActiveQuest == "carry_bar") then
...
if (CLIENT) then
    hook.Add("PostDrawTranslucentRenderables", "DrawSphereBARCarry", function(bDepth, bSkybox)
        if (bSkybox) then return end
        if not (LocalPlayer().QUEST_SYSTEM_ActiveQuest == "carry_bar") then return end
