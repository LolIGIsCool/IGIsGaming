    QUEST.Name = "Guard NPCC"
    QUEST.Instruction = "Guard the NPCC for 5 minutes." -- %s will equal the amount
QUEST.Info = ""
...
QUEST.Penalty = 250 -- penalty reward
    ...
    if (SERVER) then
        timer.Create("isguardingnpcc", 1, 0, function()
            for k, v in pairs(ents.FindInSphere(Vector(468.71, -12683, 2176), 300)) do
                if (v:IsPlayer() and v.QUEST_SYSTEM_ActiveQuest == "guard_npcc") then
