QUEST.Name = "Get the Intel"
QUEST.Instruction = "Beat a suspect up to obtain information" -- %s will equal the amount
QUEST.Reward = 500 -- credit reward
QUEST.Time = 600 -- how long, in seconds, a player has to complete the quest. 0 = infinite
QUEST.Amount = 50 -- how many actions required to complete quest
QUEST.Penalty = 50 -- penalty reward
QUEST.Info = "Dont be silly you know how to beat people up"
...
function QUEST:OnFinished(player)
    ...
    player:StripWeapon("weapon_fists")
end
...
if (SERVER) then
    hook.Add("fistsforquest", "interrogationquest", function(player, dmgamount)
        if (player:IsPlayer() and player.QUEST_SYSTEM_ActiveQuest == "interrogate") then
            player:QUEST_SYSTEM_Quest_UpdateProgress(player.QUEST_SYSTEM_QuestProgress + dmgamount)
    ...
    hook.Add("PlayerDeath", "interrogationquestdeath", function(player)
        if (player:IsPlayer() and player.QUEST_SYSTEM_ActiveQuest == "interrogate") then
            player:QUEST_SYSTEM_ChatNotify("Quest", "You cant even interrogate someone without dieing.")
            player:QUEST_SYSTEM_Quest_Abort()
        end
    end)
end
