QUEST.Name = "Naval Duties"
QUEST.Instruction = "Operate the Bridge or Control room for 10 minutes." -- %s will equal the amount
...
QUEST.Amount = 660 -- how many actions required to complete quest
QUEST.Penalty = 120 -- penalty reward
...
if (SERVER) then
    timer.Create("isguardingnavbridge", 1, 0, function()
        for k, v in pairs(ents.FindInSphere(Vector(-7930.13, -2.58, 688.96), 400)) do
            if (v:IsPlayer() and v.QUEST_SYSTEM_ActiveQuest == "navy") then
    ...
    timer.Create("isguardingnavctrlrm", 1, 0, function()
        for k, v in pairs(ents.FindInSphere(Vector(506.59, 3.72, -4799.96), 400)) do
            if (v:IsPlayer() and v.QUEST_SYSTEM_ActiveQuest == "navy") then
                v:QUEST_SYSTEM_Quest_UpdateProgress(v.QUEST_SYSTEM_QuestProgress + 1)
