QUEST.Name = "High Defender"
QUEST.Instruction = "Guard the bridge for 10 minutes." -- %s will equal the amount
...
if (SERVER) then
    timer.Create("isguardingbridge", 1, 0, function()
        for k, v in pairs(ents.FindInSphere(Vector(-9575.49, 20.81, 150.41), 300)) do
            if (v:IsPlayer() and v.QUEST_SYSTEM_ActiveQuest == "guard_bridge") then
