QUEST.Name = "Sneaky, Sneaky..."
QUEST.Instruction = "Test out your cloaking device by moving around whilst cloaked." -- %s will equal the amount
QUEST.Info = "Cloak whilst moving"
QUEST.Reward = 700 -- credit reward
QUEST.Time = 420 -- how long, in seconds, a player has to complete the quest. 0 = infinite
QUEST.Amount = 300 -- how many actions required to complete quest
...
if (SERVER) then
    hook.Add("Move", "movecamoforquest", function(ply, move)
        if ply:GetVelocity():Length() > 5 and ply:GetNWBool("CamoEnabled") then
            if (ply:IsPlayer() and ply.QUEST_SYSTEM_ActiveQuest == "cloakmove") then
