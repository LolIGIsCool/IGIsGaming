...
QUEST.Instruction = "Take this box of spare parts to the Engineer Store Room." -- %s will equal the amount
...
local pos = Vector(4934.21, -1457.63, -5303.96)

if (SERVER) then
    timer.Create("isiteminzoneENG", 1, 0, function()
        for k, v in pairs(ents.FindInSphere(Vector(pos), 150)) do
            if (v:GetClass() == "carry_item") then
                ...
                if (playertoreward.QUEST_SYSTEM_ActiveQuest == "carry_engstore") then
...
if (CLIENT) then
    hook.Add("PostDrawTranslucentRenderables", "DrawSphereENGCarry", function(bDepth, bSkybox)
        if (bSkybox) then return end
        if not (LocalPlayer().QUEST_SYSTEM_ActiveQuest == "carry_engstore") then return end
