QUEST.Name = "Unknown Box"
QUEST.Instruction = "I need you to drop this box off somewhere, I can only give you a hint though." -- %s will equal the amount
QUEST.Info = "Take the box to where the hint may suggest, HINT: Those that go there plan to die, unless they plan it for someone else"
...
function QUEST:OnStarted(player, strQuest_ID, acceptedOnline)
    ...
    player:QUEST_SYSTEM_ChatNotify("Quest", "HINT: Those that go there plan to die, unless they plan it for someone else")
...
local pos = Vector(5039.863, -412.338, -5644.187)

if (SERVER) then
    timer.Create("isiteminzoneSECRET", 3, 0, function()
        for k, v in pairs(ents.FindInSphere(Vector(pos), 150)) do
            if (v:GetClass() == "carry_item") then
                ...
                if (playertoreward.QUEST_SYSTEM_ActiveQuest == "carry_secret") then
