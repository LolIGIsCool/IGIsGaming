QUEST.Name = "Extremely Battle Ready"
QUEST.Instruction = "Head over to the fireing range and hit 25 bullseyes" -- %s will equal the amount
QUEST.Info = "The firing range can be found in the recruit room, hit 25 bullseyes"
...
if (SERVER) then
    hook.Add("EntityTakeDamage", "fireingrange25quest", function(ent, dmginfo)
        local targetname = ent:GetName()
        local ply = dmginfo:GetAttacker()

        if (targetname == "shoot1Trgt2" or targetname == "shoot2Trgt2" or targetname == "shoot3Trgt2" or targetname == "shoot4Trgt2") then
            ply:QUEST_SYSTEM_Quest_UpdateProgress(0)
        end

        if (targetname == "shoot1Trgt" or targetname == "shoot2Trgt" or targetname == "shoot3Trgt" or targetname == "shoot4Trgt") then
            if (ply:IsPlayer() and ply.QUEST_SYSTEM_ActiveQuest == "fireingrange_25") then
