QUEST.Name = "Part Transportation"
QUEST.Instruction = "Take this shipment of spare parts to MH1." -- %s will equal the amount
QUEST.Info = "Take the shipment to MH1, look for the blue sphere."
...
local pos = Vector(-1376.27, 603.64, -5951.95)

if (SERVER) then
    timer.Create("isiteminzonemh1", 1, 0, function()
        for k, v in pairs(ents.FindInSphere(Vector(pos), 150)) do
            if (v:GetClass() == "carry_item") then
                ...
                if (playertoreward.QUEST_SYSTEM_ActiveQuest == "carry_parts") then
...
if (CLIENT) then
    hook.Add("PostDrawTranslucentRenderables", "DrawSphereMH1Carry", function(bDepth, bSkybox)
        if (bSkybox) then return end
        if not (LocalPlayer().QUEST_SYSTEM_ActiveQuest == "carry_parts") then return end
