QUEST.Name = "Leg Cramps"
QUEST.Instruction = "Crouch walk for 200 metres" -- %s will equal the amount
QUEST.Info = "Crouch walk for 200 metres, what else can I say?"
QUEST.Reward = 700 -- credit reward
QUEST.Time = 420 -- how long, in seconds, a player has to complete the quest. 0 = infinite
QUEST.Amount = 200 -- how many actions required to complete quest
QUEST.Penalty = 70 -- penalty reward
...
if (SERVER) then
    hook.Add("Move", "TrooperCrouchQuest", function(ply, move)
        if move:KeyDown(IN_DUCK) and ply:GetVelocity():Length() > 10 then
            if (ply:IsPlayer() and ply.QUEST_SYSTEM_ActiveQuest == "trooper_crouch") then
                ply:QUEST_SYSTEM_Quest_UpdateProgress(ply.QUEST_SYSTEM_QuestProgress + 0.2)
