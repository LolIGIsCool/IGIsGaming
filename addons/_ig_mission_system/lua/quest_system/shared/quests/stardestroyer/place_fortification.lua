QUEST.Name = "Deploy Fortifications"
QUEST.Instruction = "Place down fortifications for the protection of the troopers." -- %s will equal the amount
QUEST.Info = "Place 10 fortifications using the fortification tool."
QUEST.Reward = 500 -- credit reward
QUEST.Time = 240 -- how long, in seconds, a player has to complete the quest. 0 = infinite
QUEST.Amount = 10 -- how many actions required to complete quest
...
if (SERVER) then
    hook.Add("placefortification", "registerforificationquest", function(player)
        if (player:IsPlayer() and player.QUEST_SYSTEM_ActiveQuest == "place_fortification") then
