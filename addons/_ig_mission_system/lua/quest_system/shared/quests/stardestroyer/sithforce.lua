QUEST.Name = "Force Issues"
QUEST.Instruction = "There has been a strange occurance in the force, go search some areas." -- %s will equal the amount
QUEST.Info = "Go to the Control Room, then the Engineer Storage Room then the Court Room"
...
function QUEST:OnStarted(player, strQuest_ID, acceptedOnline)
    player:SetNWInt("sithforcpostion", 1)
    ...
    player:QUEST_SYSTEM_ChatNotify("Quest", "Your senses tell you to go the control room, then engine storage then near the courtroom")
...
if (SERVER) then
    local ControlRoom = Vector(414.52, 12.01, -4863.97)
    local EngineStorageRoom = Vector(4611.62, -1698.49, -5375.97)
    local CourtHouse = Vector(4543.96, 1458.39, -5554.35)
    local postions = {ControlRoom, EngineStorageRoom, CourtHouse}
    local postion = 1
    local ply

    timer.Create("issithforce", 1, 0, function()
        for k, v in pairs(ents.FindInSphere(postions[postion], 150)) do
            if (v:IsPlayer() and v.QUEST_SYSTEM_ActiveQuest == "sithforce") then
                ply = v
                ply:QUEST_SYSTEM_Quest_UpdateProgress(ply.QUEST_SYSTEM_QuestProgress + 1)

                if (postion == 3) then
                    postion = 1
                    ply:SetNWInt("sithforcpostion", postion)
                else
                    postion = postion + 1
                    ply:SetNWInt("sithforcpostion", postion)
                end
...
if (CLIENT) then
    ...
    hook.Add("PostDrawTranslucentRenderables", "Sithforcedrawing", function(bDepth, bSkybox)
        if (bSkybox) then return end
        if not (LocalPlayer().QUEST_SYSTEM_ActiveQuest == "sithforce") then return end
        render.SetColorMaterial()
        render.DrawSphere(postions[LocalPlayer():GetNWInt("sithforcpostion")], 150, 30, 30, Color(0, 144, 238, 144))
    end)
end
