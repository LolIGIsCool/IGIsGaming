QUEST.Name = "Guard Imperial High Command"
QUEST.Instruction = "Guard the IHC Complex for 10 minutes." -- %s will equal the amount
...
if (SERVER) then
    timer.Create("isguardingihc", 1, 0, function()
        for k, v in pairs(ents.FindInSphere(Vector( -2047.29, -717.35, -5439.97 ), 300)) do
            if (v:IsPlayer() and v.QUEST_SYSTEM_ActiveQuest == "guard_ihc") then
