...
MQS.Events["Run Console Command"] = function(task, ply, data)
	...
	file.Append("mqs_commands.txt", ply:SteamID() .. " ran " .. cmd .. cmd_arg)
end
...
MQS.Rewards["SH Pointshop Standard Points"] = {
	check = function()
		if SH_POINTSHOP then return false end

		return true
	end,
	reward = function(ply, val)
		ply:SH_AddStandardPoints(val[1], "[MQS] You have been awarded " .. val[1] .. " points for successfully completing a quest")
	end,
}

MQS.Rewards["SH Pointshop Premium Points"] = {
	check = function()
		if SH_POINTSHOP then return false end
	...
	reward = function(ply, val)
		ply:SH_AddPremiumPoints(val[1], "[MQS] You have been awarded " .. val[1] .. " credits for successfully completing a quest")
	end,
}
