...
function ENT:OnRemove()
	if self.afterjump == true then
		...
		if self.FreezeValue == 1 then
			local phys = afterjump:GetPhysicsObject()
			phys:EnableMotion(false)
		end
		if self.AIValue == 1 and self.SpawnModelValue == 0 then
			afterjump:SetAI(true)
			if self.TeamValue ~= 4 then
				afterjump:SetAITEAM(self.TeamValue)
			end
		end
