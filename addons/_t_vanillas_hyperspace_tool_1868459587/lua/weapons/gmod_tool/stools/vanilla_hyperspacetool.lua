...
function TOOL:LeftClick( trace, attach )
    ...
    local Team = self:GetClientInfo("team")
    ...
    if Valid == true then
        timer.Simple(Delay, function()
            local ent = ents.Create("IGvanilla_hyperspace_ship")
...
function TOOL:RightClick( trace )
	...
	if ent:IsPlayer() and not self:GetOwner():IsSuperAdmin() then return end
    local sound = ents.Create("IGvanilla_highwake")
...
function TOOL.BuildCPanel( CPanel )
    ...
    CPanel:NumSlider("Team", "vanilla_hyperspacetool_team", 0, 4, 0)
    CPanel:ControlHelp("Sets the team of the ship. (4 is default team)")
