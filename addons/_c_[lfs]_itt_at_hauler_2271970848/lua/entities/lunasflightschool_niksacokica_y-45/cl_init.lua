...
function ENT:EngineActiveChanged( bActive )
	if bActive then
		self.ENG = CreateSound( self, "niksacokica/jfo_vehicles/y-45_engine_loop.wav" )
		self.ENG:PlayEx(0,0)
	else
		self:SoundStop()
	end
...
function ENT:Draw()
	...
	local r = Color( 0, 0, 255, 255)

	render.SetMaterial(Material( "sprites/light_glow02_add" ))
	for j=0, 1 do
		for i=0, 3 do
			render.DrawSprite( self:LocalToWorld( Vector(-366,250-500*j,143-2*i) ), Size, Size, r )
			
			render.DrawSprite( self:LocalToWorld( Vector(-366,250-500*j,124-2*i) ), Size, Size, r )
			
			render.DrawSprite( self:LocalToWorld( Vector(-366,250-500*j,105-2*i) ), Size, Size, r )
		end
	end
end