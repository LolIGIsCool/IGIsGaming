...
function SWEP:PrimaryAttack()
    ...
    for k,v in pairs(player.GetAll()) do
        local maxhealth = v:GetMaxHealth() + (v:GetMaxHealth() * 0.4) -- Max HP to heal could be set job specific, create a table to reference further up and put it here with an or statement
		if SERVER and _G.HasAugment(self.Owner,"Stimulant Addict") then
			maxhealth = v:GetMaxHealth() + (v:GetMaxHealth() * 0.45);
		end
...
function SWEP:SecondaryAttack()
	...
	if SERVER and _G.HasAugment(self.Owner,"Stimulant Addict") then
		maxhealth = self.Owner:GetMaxHealth() + (self.Owner:GetMaxHealth() * 0.45);
	end
