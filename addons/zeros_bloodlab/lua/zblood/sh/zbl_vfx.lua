...
zbl.NetEffectGroups = {
	...
	["player_disinfect"] = {
		...
		action = function(ply)
			...
			if entityClass == "gmod_cameraprop" or (entityClass == "player" and GetConVar( "simple_thirdperson_enabled" ) and GetConVar( "simple_thirdperson_enabled" ):GetBool()) then
				...
				local bone = (ply:LookupBone("ValveBiped.Bip01_R_Hand") or 1)
				local _pos,_ang = ply:GetBonePosition(bone)
				if not _pos or not _ang then return end
	...
	["fence_disinfect"] = {
		...
		action = function(fence)

			fence:EmitSound("zbl_spray")

			zbl.f.ParticleEffect("zbl_disinfect", fence:GetPos(),fence:GetAngles() ,fence)
		end,
	},
...
function zbl.f.Destruct(pos,effect)
	local vPoint = pos
	local effectdata = EffectData()
