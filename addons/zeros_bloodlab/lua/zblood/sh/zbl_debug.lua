...
if SERVER then
	...
	*/
	...
	concommand.Add("zbl_debug_vaccinate_self", function(ply, cmd, args)
	    if IsValid(ply) and zbl.f.IsAdmin(ply) then
	        local vaccineID = tonumber(args[1]) or 1
	        local vaccineStage = tonumber(args[2]) or 1
	        ...
	        if vaccineID and vacData and vacData.mutation_stages and vaccineStage <= #vacData.mutation_stages then
				zbl.f.Player_Vaccinate(ply,vaccineID,vaccineStage,ply)
	        end
	    end
	end)
	...
	concommand.Add("zbl_debug_vaccinate_target", function(ply, cmd, args)
	    if IsValid(ply) and zbl.f.IsAdmin(ply) then
	        ...
	        if tr.Hit and IsValid(tr.Entity) and tr.Entity:IsPlayer() and tr.Entity:Alive() then
	            ...
	            if vaccineID then
	                zbl.f.Player_Vaccinate(tr.Entity,vaccineID,vaccineStage,ply)
	            end
	        end
	    end
	end)
	    ...
	    if IsValid(ply) and zbl.f.IsAdmin(ply) then
	        ...
	        if vaccineID then
	            for k,v in pairs(zbl_PlayerList) do
	                if IsValid(v) then
	                    zbl.f.Player_Vaccinate(v,vaccineID,vaccineStage,ply)
	                end
	            end
	        end
	    end
	...
	concommand.Add("zbl_debug_corpse_graveyard", function(ply, cmd, args)
		if IsValid(ply) and zbl.f.IsAdmin(ply) then
