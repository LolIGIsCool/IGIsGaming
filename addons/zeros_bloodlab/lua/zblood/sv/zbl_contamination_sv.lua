...
function zbl.f.Ctmn_ObjectContaminate(Object,virus_id)
    ...
    // If we allready reached the contaminated object limit then lets stop for now
    if zbl.ContaminatedObjects and table.Count(zbl.ContaminatedObjects) >= zbl.config.Contamination.limit then
        return
    end
...
hook.Add( "PlayerUse", "zbl_PlayerUse", function( ply, ent )
    if zbl.config.Contamination.enabled and IsValid(ent) and IsValid(ply) and (ply.zbl_NextObjectUse == nil or ply.zbl_NextObjectUse < CurTime()) and zbl.f.Ctmn_CanBeContaminated(ent) then
        ...
        if zbl.config.Contamination.AutoContaminate.enabled and zbl.config.Contamination.AutoContaminate.HeavyUsePriority and medics == true then
...
timer.Create("MedicCheck", 5, 0, function()
    medictable = {}
    ...
    if table.Count(medictable) <= 0 then
        medics = false
    elseif table.Count(medictable) > 3 then
        medics = true
    end
end)
////////////////////////////////////////////
