...
zbl.Symptomes["coughing"] = {
    ...
    OnStart = function(ply,data)
        ...
        zbl.f.Timer_Create(timerid,data.interval,0,function()
            ...
            if zbl.f.RandomChance(75) == false then
                return
            end
            ...
            local chance , _ = zbl.f.Player_ProtectionTest(ply, vac_id, vac_stage)
            chance = (1 / 100) * chance
...
function zbl.f.Symptome_OnStart(ply,vaccineID,vaccineStage)
    ...
    // Add ragdoll
    if mutation_stage.effects and mutation_stage.effects["ragdoll"] then
        RunConsoleCommand("ulx","ragdoll",ply:Nick())
    end
    // Add heal
    if mutation_stage.effects and mutation_stage.effects["regen"] then
        timer.Create(ply:SteamID().."ZBLRegen",2,0,function()
          if ply:Health() + mutation_stage.effects["regen"] >= ply:GetMaxHealth() then
            ply:SetHealth(ply:GetMaxHealth())
          else
            ply:SetHealth(ply:Health() + mutation_stage.effects["regen"])
            if ply:Health() <= 0 then
              ply:Kill()
            end
          end
        end)
        timer.Start(ply:SteamID().."ZBLRegen")
    end
end
...
function zbl.f.Symptome_OnEnd(ply)
    ...
    // Turn off heal !!!
    if mutation_stage.effects and mutation_stage.effects["regen"] then
        timer.Destroy(ply:SteamID().."ZBLRegen")
    end
end
