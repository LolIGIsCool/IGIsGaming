...
/*
hook.Add("PlayerDeath", "COVID_PlayerDeath", function(victim, inflictor, attacker)
    if zbl.f.Player_HasVaccine(victim) then
        local vac_id = victim:GetNWInt("zbl_Vaccine", -1)

        -- 3 = COVID19
        if vac_id == 3 then
            -- lETS CHANGE THE JOB TO CORONA CHAN!
            victim:changeTeam(TEAM_CORONACHAN, true, true)
        end
    end
end)
*/
...
hook.Add("IGPlayerSay", "zbl_PlayerSay", function(ply, text)
