...
function zbl.f.Injector_SetupFlasks(swep)
	...
	zbl.f.Debug("zbl.f.Injector_SetupFlasks")
...
function zbl.f.Injector_UpdateFlaskSequence(swep)
	...
	zbl.f.Debug("zbl.f.Injector_UpdateFlaskSequence: " .. sequence)
...
function zbl.f.Injector_Secondary(swep)
	...
	if tr.Hit and IsValid(ent) and zbl.f.InDistance(swep.Owner:GetPos(), ent:GetPos(), 100) then
		...
		if ent:GetClass() == "zbl_flask" and ent:GetGenType() ~= 0 then
			...
			zbl.f.Injector_PlayShootAnim(swep)
		elseif zbl.config.Contamination.collectable == true and zbl.config.Contamination.ents[ent:GetClass()] and zbl.f.Ctmn_IsObjectContaminated(ent) then

			local sample_type = zbl.config.SampleTypes["contaminated_objects"]
			...
			zbl.f.Injector_AddLiquid(swep,empty_flask,1,sample_value,sample_name,sample_points,ent:GetClass())
