...
function zbl.f.Player_GetProtectionChance(ply, vaccineID, vaccinestage)
    return math.Clamp(zbl.config.ProtectionCheck(ply, vaccineID, vaccinestage),0,100)
end
...
function zbl.f.Player_ProtectionTest(ply, vaccineID, vaccinestage)
    ...
    // Just to be sure if 100% protection chance then lets set him to immun
    if chance >= 100 then
        immun = true
    end
...
function zbl.f.Player_Infect(ply,vaccineID,vaccinestage)
    ...
    // This is a simple version of zbl.config.ProtectionCheck to block anything
    local InfectionBlock = hook.Run("zbl_BlockInfection" ,ply,vaccineID)
    if InfectionBlock then
        return
    end
    ...
    local _ , immun = zbl.f.Player_ProtectionTest(ply, vaccineID, vaccinestage)
    if immun then
...
function zbl.f.Player_ApplyVaccineStage(ply,vaccineID,vaccinestage)
    ...
    local stage_duration = vaccineData.duration


    // If there is a duration value defined in the mutation stage then we override the default vaccine duration
    if vaccineData.mutation_stages[vaccinestage] and vaccineData.mutation_stages[vaccinestage].duration then
        stage_duration = vaccineData.mutation_stages[vaccinestage].duration
    end

    zbl.f.Timer_Create(timerid,stage_duration,1,function()
