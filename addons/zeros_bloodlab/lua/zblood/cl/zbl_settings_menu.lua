...
CreateConVar("zbl_cl_fence_simplify", "0", {FCVAR_ARCHIVE})
	...
	for k, v in pairs(cmds) do
		...
		elseif v.class == "DButton" then
			...
			item.DoClick = function()
				...
				if zbl.f.IsSuperAdmin(LocalPlayer()) == false then return end
...
local function BloodLab_settings(CPanel)
	...
	zbl_OptionPanel("VFX",CPanel,{
		...
		[5] = {name = "Simplify Fence",class = "DCheckBoxLabel", cmd = "zbl_cl_fence_simplify"},
	})
