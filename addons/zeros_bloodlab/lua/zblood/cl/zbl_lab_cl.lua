...
function zbl.f.Lab_MainMenu(Lab)
	...
	Lab.VGUI.Mainmenu.Research.DoClick = function()
		print(zbl.f.IsOwner(LocalPlayer(), Lab))
		if zbl.f.IsOwner(LocalPlayer(), Lab) == false then
			surface.PlaySound("common/warning.wav")
			print("test")
			notification.AddLegacy(zbl.language.General["Wrong Owner"], NOTIFY_ERROR, 3)
...
function zbl.f.Lab_VaccineList(Lab,starting_id)
	...
	for i = 1,table.Count(zbl.config.Vaccines) do
		...
		Lab.VGUI.Research.ResearchItems[i].Paint = function(s, w, h)
			...
			if vax_data.research and vax_data.research["ranks"] and table.Count(vax_data.research["ranks"]) > 0 and vax_data.research["ranks"][zbl.f.GetPlayerRank(LocalPlayer())] == nil then
				surface.SetDrawColor(zbl.default_colors["white01"])
				surface.SetMaterial(zbl.default_materials["zbl_icon_locked"])
				surface.DrawTexturedRect(20, 5, 50, 50)
			else
				if vax_data.isvirus then
					surface.SetDrawColor(zbl.default_colors["white01"])
		...
		Lab.VGUI.Research.ResearchItems[i].RButton.DoClick = function()
			...
			if zbl.f.Lab_RankCheck(i,true) == false then return end
...
// A quick function which informs the user if he can create this vaccine
function zbl.f.Lab_RankCheck(vac_id,notify)
	local vaccine_data = zbl.config.Vaccines[vac_id]

	if vaccine_data == nil then return false end
...
function zbl.f.Lab_CreateVaccine(Lab)
	...
	if zbl.f.Lab_RankCheck(Lab.VaccineID,true) == false then return end
