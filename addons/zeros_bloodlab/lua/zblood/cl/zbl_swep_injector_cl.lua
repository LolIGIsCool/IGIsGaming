...
function zbl.f.Injector_Initialize(swep)
	...
	zbl.f.Injector_FlaskSequenceChanged(swep,swep:GetFlaskSequence())
end
...
function zbl.f.Injector_FlaskSequenceChanged(swep,s_new_sequence)
	if not isstring(s_new_sequence) then return end
	zbl.f.Debug("zbl.f.Injector_FlaskSequenceChanged: " .. s_new_sequence)
	local seq_tbl = string.Split(s_new_sequence,"_" )
	swep.FlaskSequence = {}

	if seq_tbl and #seq_tbl > 0 then
		for k,v in pairs(seq_tbl) do
			local val = tonumber(v)

			// If for some reason the flask value is nill then lets set it to 0 (EMPTY)
			if val == nil then
				val = 0
			end

			swep.FlaskSequence[k] = val
		end
	end
end
...
local function DrawFlaskIcon(swep,color, pos,_selected)
...
local function DrawFlaskCount(swep)
	local _selected = swep:GetSelectedFlask()

	if swep.FlaskSequence and table.Count(swep.FlaskSequence) > 0 then
		for k,v in pairs(swep.FlaskSequence) do

			if v > 100 then
				// Ability
				DrawFlaskIcon(swep,zbl.default_colors["abillity_yellow"], k,_selected)
			else
				if v == 0 then
					// Empty
					DrawFlaskIcon(swep,zbl.default_colors["grey01"], k,_selected)
				elseif v == 1 then
					//Sample
					DrawFlaskIcon(swep,zbl.default_colors["sample_blue"], k,_selected)
				elseif v == 2 then
					//Virus
					DrawFlaskIcon(swep,zbl.default_colors["virus_red"], k,_selected)
				elseif v == 3 then
					// Cure
					DrawFlaskIcon(swep,zbl.default_colors["cure_green"], k,_selected)
				end
			end
		end
	end
