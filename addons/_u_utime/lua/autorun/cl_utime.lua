...
local ContextUtimeButton

hook.Add("OnContextMenuOpen", "contextmenuopenutime", function()

ContextUtimeButton = true

end)

hook.Add("OnContextMenuClose", "contextmenucloseutime", function()

ContextUtimeButton = false
...
function think()
	...
	if not utime_enable:GetBool() or not IsValid( LocalPlayer() ) or
			...
			or not ContextUtimeButton then
...
function PANEL:ShouldRevealPlayer( ply )
	...
	if ply:GetNWBool("CamoEnabled", false) then -- Cloak
		return false
	end
