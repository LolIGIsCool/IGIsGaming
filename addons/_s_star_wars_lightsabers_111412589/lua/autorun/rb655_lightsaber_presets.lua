...
local colourtbl = {["red"] = Color(255,0,0),
				   ["green"] = Color(0,255,0),
				   ["white"] = color_white
					}
local chriscolour = colourtbl["red"]

function rb655_RenderBlade( pos, dir, len, maxlen, width, color, black_inner, eid, underwater, quillon, bladeNum, moose, martibo, kumo, saberlvl )
	...
	if ( kumo ) and saberlvl ~= 1 and saberlvl ~= 2 then
		--[[maxlen = maxlen * 2
		len = len * 2
		width = width * 2]]
		color = HSVToColor((CurTime() * 30)%360,1,1)
		--color = Color(math.random(0,255),math.random(0,255),math.random(0,255))
		--[[if chriscolour == colourtbl["red"] then
			chriscolour = colourtbl["green"]
		elseif chriscolour == colourtbl["green"] then
			chriscolour = colourtbl["white"]
		elseif chriscolour == colourtbl["white"] then
			chriscolour = colourtbl["red"]
		end]]
		--color = chriscolour
		local ed = EffectData()
		ed:SetOrigin( pos )
		...
		ed2:SetNormal( dir )
		ed2:SetRadius( len )
		ed2:SetAngles( Angle( color.r, color.g, color.b ) )
		util.Effect( "wos_unstable_discharge", ed2 )	
	end
	
	local inner_color = color_white
	if ( black_inner ) then inner_color = Color( 0, 0, 0 ) end
	--if ( black_inner ) and ( moose ) then inner_color = Color( 255, 0, 0, 0 ) end
	...
	if ( !quillon ) then
		...
		if ( SaberLight ) then
			...
			SaberLight.Size = 300 * ( len / maxlen )
			SaberLight.Decay = 0
