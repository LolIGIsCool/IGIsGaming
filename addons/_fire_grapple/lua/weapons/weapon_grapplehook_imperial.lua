...
CreateConVar("grapple_distance", 1250, bit.bor(FCVAR_GAMEDLL, FCVAR_NOTIFY, FCVAR_SERVER_CAN_EXECUTE),"max distance which its allowed to fire")
CreateConVar("grapple_latch_speed", 4000, bit.bor(FCVAR_GAMEDLL, FCVAR_NOTIFY, FCVAR_SERVER_CAN_EXECUTE),"speed at which the rope flies to its target")
...
local sndGrappleHit		= Sound("weapons/fire_grappling_hook_impact_default.wav")
local sndGrappleHitFlesh= Sound("weapons/grappling_hook_impact_flesh.wav")
local sndGrappleShoot	= Sound("weapons/fire_grappling_hook_shoot.wav")
local sndGrappleReel	= Sound("weapons/fire_grappling_hook_reel_start.wav")
...
-- SWEP.VElements = {
-- -- 	["v_element"] = { type = "Model", model = "models/sw_battlefront/weapons/dc17_blaster.mdl", bone = "ValveBiped.Bip01_R_Hand", rel = "", pos = Vector(3.032, 1.476, -1.58), angle = Angle(0, 0, 177.921), size = Vector(1, 1, 1), color = Color(255, 255, 255, 255), surpresslightning = false, material = "", skin = 0, bodygroup = {} }
...
-- SWEP.WElements = {
-- -- 	["w_element"] = { type = "Model", model = "models/sw_battlefront/weapons/dc17_blaster.mdl", bone = "ValveBiped.Bip01_R_Hand", rel = "", pos = Vector(3.709, 1.886, -2), angle = Angle(0, 0, 180), size = Vector(1, 1, 1), color = Color(255, 255, 255, 255), surpresslightning = false, material = "", skin = 0, bodygroup = {} }
-- }
...
function SWEP:Think()
	...
	if ( self.Owner:KeyPressed( IN_ATTACK ) ) then
		...
		-- 	self.Idle = false
		-- 	-- set idle timer anyway
		-- 	self.IdleTimer = CurTime() + 3
		-- else
		-- 	-- if (CLIENT) then
		-- 	-- 	self.Weapon:EmitSound( sndGrappleAbort,40 )
		-- 	-- end
		-- end
		-- self:StartAttack()
		-- self.VElements["v_element"].material = "models/weapons/c_items/c_grappling_hook_alphatest"
	elseif ( self.Owner:KeyDown( IN_ATTACK ) && inRange && !!self.Beam ) then
		self:UpdateAttack()
...
function SWEP:UpdateAttack()
	...
	if ( self.Tr.Entity:IsValid() ) then
		...
		if SERVER then
            if self.Tr.Entity:IsPlayer() then self:EndAttack(false) return false end
			self.Beam:GetTable():SetEndPos( endpos )
	...
	if(self.dt < 0) then
		if GetConVarNumber("grapple_play_sounds") == 1 then
			self.Weapon:EmitSound(sndGrappleHit, 50 )
	...
	if(self.dt == 0) then
		...
		if GetConVarNumber("grapple_play_sounds") == 1 then
			-- self.Weapon:EmitSound( sndGrappleShoot,75 )
			curVel = self.Owner:GetVelocity()
			if (math.max(math.abs(curVel.x), math.abs(curVel.y), math.abs(curVel.z)) > GetConVarNumber("grapple_speed_mk2") / 2) then
				self.ReelIn = CreateSound(self.Owner, "weapons/fire_grappling_hook_reel_start.wav")
				self.ReelIn:Play()
			end
		end
...
--        local timeleft = self.IdleTimer - CurTime()
        --print(self.IdleTimer, CurTime(), timeleft)
         --_G.vanillaBlurPanel(ScrW() / 2 - Width / 2, ScrH() - Height * 2, Width, Height, Color(25, 255, 125, 100))
--       	if timeleft > 0 then
--            _G.vanillaBlurPanel(ScrW() / 2 - Width / 2, ScrH() - Height * 2, Width, Height, Color(25, 255, 125, 100))
--        end
--    end
