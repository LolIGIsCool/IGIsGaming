...
SWEP.PrintName						= "HenDoge's Laser Beam"
...
SWEP.VElements = {
	...
	["shotgun_module"] = { type = "Model", model = "models/cs574/dc17m/dc17m_shotgun.mdl", bone = "", rel = "", pos = Vector(-1.3, 13, -0.3), angle = Angle(0, 0, 0), size = Vector(0.8, 0.8, 0.8), color = Color(255, 255, 255, 255), surpresslightning = false, material = "", skin = 0, bodygroup = {}, active = false  },
}
...
SWEP.WElements = {
	...
	["shotgun_module"] = { type = "Model", model = "models/cs574/dc17m/dc17m_shotgun.mdl", bone = "ValveBiped.Bip01_R_Hand", rel = "e11d", pos = Vector(0, 8, 0), angle = Angle(0, 0, 0), size = Vector(0.6, 0.6, 0.6), color = Color(255, 255, 255, 255), surpresslightning = false, material = "", skin = 0, bodygroup = {}, active = false  },
}
...
SWEP.Attachments = {
	...
	[7] = { offset = { 0, 0 }, atts = { "powerpack","dc17m_magext"}, order = 7 },
	[8] = { offset = { 0, 0 }, atts = { "dc17m_sniper","dc17m_shotgun","dc17m_rocket"}, order = 8 },
}
...
function SWEP:Equip()
    if not (self.Owner:IsSuperAdmin() or self.Owner:IsUserGroup("Senior Event Master") or self.Owner:IsUserGroup("Lead Event Master")  or self.Owner:IsUserGroup("admin") or self.Owner:IsUserGroup("Senior Developer") or self.Owner:IsUserGroup("Developer")) then
	    ...
	    self.Owner:PrintMessage(HUD_PRINTTALK, "You somehow obtained HenDoge's Laser Beam. It has been removed.")
	    self.Owner:StripWeapon("rw_sw_e11dt")

	    if SERVER then
	    	for k, v in ipairs(player.GetAll()) do
	        	if v:IsSuperAdmin() then
	            	v:PrintMessage(HUD_PRINTTALK, "[DevWepAlert] "..notyboi.." | Was Given an 'rw_sw_e11dt', Discord Relay or Echos will show how it was Obtained.|")
	       		end
	    	end
    	end
	...
	elseif SERVER then 
		if (self.Owner:IsUserGroup("Senior Event Master") or self.Owner:IsUserGroup("Lead Event Master")  or self.Owner:IsUserGroup("admin")) then
			for k, v in ipairs(player.GetAll()) do
			    ...
			    if v:IsSuperAdmin() then
					v:PrintMessage(HUD_PRINTTALK, "[DevWepAlert] "..godyboi.." Has obtained the 'HenDoge's Laser' Dev Weapon.")
			    end
			end
    	end
	end
