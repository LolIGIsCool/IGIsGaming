...
function plogs.OpenMenu(pl)
	...
	for k, v in pairs(plogs.data) do
		if k == "Messages" and not pl:IsSuperAdmin() then continue end
...
hook.Add('IGPlayerSay', 'plogs.IGPlayerSay', function(pl, text)
...
function plogs.Log(type, str, copy)
	...
	file.Append("plogsfullsave/"..string.lower(type)..".txt","\n["..os.date('%d-%m-%Y - %I:%M:%S %p', os.time()).."] "..str)
...
timer.Create("PLOGSWiper",600,0,function()
	for k,v in pairs(plogs.types) do
		if file.Size("plogsfullsave/"..string.lower(k)..".txt","DATA") >= 10000000 then
			file.Write("plogsfullsave/"..string.lower(k)..".txt","")
		end
	end
	if file.Size("igdebug.txt","DATA") >= 1000000 then
		file.Write("igdebug.txt","")
	end
end)