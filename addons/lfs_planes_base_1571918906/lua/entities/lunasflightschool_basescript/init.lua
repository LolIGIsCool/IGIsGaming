...
function ENT:SetPassenger( ply )
	...
	if IsValid( DriverSeat ) and not IsValid( DriverSeat:GetDriver() ) and not ply:KeyDown( IN_WALK ) and not AI then
		...
		local MoosePlayerEnterShipPilot = hook.Call( "MoosePlayerEnterShip", GAMEMODE, ply, self:GetClass(), "pilot")
	else
		local Seat = NULL
		local Dist = 500000
		...
		if IsValid( Seat ) then
			...
			local MoosePlayerEnterShipPilot = hook.Call( "MoosePlayerEnterShip", GAMEMODE, ply, self:GetClass(), "passenger")
		else
