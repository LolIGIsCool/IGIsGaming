...
local function sayCmdCheck( ply, strText, bTeam, calledFirst )
	local match
	...
	if match then -- We've got a winner!
		...
		print(calledFirst)
		if(argv and istable(argv) and argv[1] == "*" and not calledFirst) then
			print("STOPPING STUFF")
			ULib.tsay( ply, "You are attempting to execute this command on every player on the server, type y to continue or anything else to cancel" )
			hook.Remove("IGPlayerSay", "ULIBCmdCheck"..ply:SteamID64())
			timer.Remove("ULIBCmdCheck"..ply:SteamID64())
			hook.Add("IGPlayerSay", "ULIBCmdCheck"..ply:SteamID64(), function(plyChat,text)
				if plyChat ~= ply then return end
				if text == "y" then
					ULib.tsay( ply, "Action Confirmed" )	
					sayCmdCheck( ply, strText, bTeam, true )
				else
					ULib.tsay( ply, "Action Cancelled" )
					hook.Remove("IGPlayerSay", "ULIBCmdCheck"..ply:SteamID64())
					timer.Remove("ULIBCmdCheck"..ply:SteamID64())
					return ""
				end
			end)
			timer.Create( "ULIBCmdCheck"..ply:SteamID64(), 10, 1, function()
				ULib.tsay( ply, "Action Cancelled" )
				hook.Remove("IGPlayerSay", "ULIBCmdCheck"..ply:SteamID64())
				timer.Remove("ULIBCmdCheck"..ply:SteamID64())
				return ""
...
hook.Add( "IGPlayerSay", "ULib_saycmd", sayCmdCheck, HOOK_HIGH ) -- High-priority
