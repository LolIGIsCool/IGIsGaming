...
if CLIENT then
    ...
    local strsnapkey = false
    local strsnaptime = false
    local strsnaplock = false
    local strsnapclick = false
    local strsnapclickfade = 0
    local strsnapcursor = false
    local strsnapspawnmenu = false
    ...
    local condefs = {
        strsnap_enabled = 1,
        strsnap_gcboost = 1,
        strsnap_gcstrength = 125,
        strsnap_hidegrid = 0,
        strsnap_clickgrid = 0,
        strsnap_toggledelay = 0,
        strsnap_disableuse = 0,
        strsnap_allentities = 0,
        strsnap_alltools = 0,
        strsnap_enabletoggle = 0,
        strsnap_lockdelay = 0.5,
        strsnap_distance = 250,
        strsnap_gridlimit = 16,
        strsnap_gridsize = 8,
        strsnap_gridalpha = 0.4,
        strsnap_gridoffset = 0.5,
        strsnap_boundingbox = 1,
        strsnap_revertaim = 1,
        strsnap_centerline = 1
    }
    ...
    local function OnInitialize()
        ...
        for _, filename in ipairs(file.Find('smartstrsnap_offsets_*.png', "GAME")) do
            ...
            if file then
                ...
                if header == "SMARTstrsnap_OFFSETS" then
    ...
    local function OnShutDown()
        output = file.Read('smartstrsnap_offsets_custom.png')

        if output == nil then
            output = "SMARTstrsnap_OFFSETS\n"
        end
        ...
        file.Write('smartstrsnap_offsets_custom.png', output)
    end
    ...
    concommand.Add("strsnap_dev_alloffset", function(player, command, arguments)
    ...
    concommand.Add("strsnap_dev_gridoffset", function(player, command, arguments)
        if target.active == true then
            if #arguments >= 1 then
                local v = GetDevOffset()
    ...
    concommand.Add("strsnap_dev_saveoffset", function(player, command, arguments)
        if target.active == true then
            local v = GetDevOffset()
    ...
    local function strsnapToggleGrid()
        if (GetConVarNumber("strsnap_enabled") == 0) then
            RunConsoleCommand('strsnap_enabled', '1')
        else
            RunConsoleCommand('strsnap_enabled', '0')
        end
    end

    local function strsnapPress()
        if GetConVarNumber("strsnap_clickgrid") ~= 0 and not strsnapclick then
            strsnapclick = true
            strsnapclickfade = CurTime()
        elseif GetConVarNumber("strsnap_clickgrid") == 0 or strsnapclick then
            if (strsnaplock or strsnapcursor) then
                strsnaptime = false
            else
                local toggledelay = GetConVarNumber("strsnap_toggledelay")

                if (toggledelay > 0 and strsnaptime and strsnaptime + toggledelay > CurTime()) then
                    strsnapToggleGrid()
                    strsnaptime = false
                    strsnaplock = false
                else
            ...
            strsnapkey = target.active

            if (not strsnapcursor) then
                strsnaplock = false
            end
        end
    end

    local function strsnapRelease()
        strsnapkey = false
    end

    local function strsnapLock()
        strsnaplock = not strsnaplock
    end

    local function OnSpawnMenu()
        strsnapspawnmenu = true
    end

    local function OnKeyPress(player, key)
        if (key == IN_USE and GetConVarNumber("strsnap_disableuse") == 0) then
            strsnapPress()
        end
    end
    ...
    local function OnThink()
        if (vgui.CursorVisible()) then
            if (not strsnapcursor and strsnaplock) then
                strsnaptarget = table.Copy(target)
            end

            strsnaptime = false
            strsnapcursor = true
        else
            if (strsnapcursor and strsnaplock) then
                target = strsnaptarget
            end

            strsnapspawnmenu = false
            strsnapcursor = false
        end

        if (GetConVarNumber("strsnap_enabletoggle") ~= 0) then
            if (strsnapkey and strsnaptime and not strsnaplock) then
                if (CurTime() > strsnaptime + GetConVarNumber("strsnap_lockdelay")) then
                    strsnaplock = true
                    strsnaptime = false
                end
            end
        ...
        target.locked = (strsnapkey or strsnaplock and not strsnapcursor) and target.active

        if (not target.locked and locked and GetConVarNumber("strsnap_revertaim") ~= 0) then
            if (strsnapcursor) then
    ...
    local function CalculateGridAxis(L)
        ...
        local grid = math.Clamp(math.floor(length / (2 * GetConVarNumber("strsnap_gridsize"))) * 2, 2, GetConVarNumber("strsnap_gridlimit"))
        local offset = math.Clamp(GetConVarNumber("strsnap_gridoffset") / length, 0, 1 / grid)
    ...
    local function Calculatestrsnap(X, Y, v)
    ...
    local function DrawGridLines(vOrigin, vSX, vSY, gridLines, offsetX, offsetY, sign)
        local centerline = (GetConVarNumber("strsnap_centerline") ~= 0)
    ...
    local function DrawGrid(vOrigin, vSX, vSY)
        ...
        surface.SetDrawColor(0, 0, 0, math.Round(GetConVarNumber("strsnap_gridalpha") * 255))
        DrawGridLines(vOrigin, vSX, vSY, LX.grid, LX.offset, LY.offset, 1)
        DrawGridLines(vOrigin, vSY, vSX, LY.grid, LY.offset, LX.offset, 1)
        surface.SetDrawColor(255, 255, 255, math.Round(GetConVarNumber("strsnap_gridalpha") * 255))
        DrawGridLines(vOrigin, vSX, vSY, LX.grid, LX.offset, LY.offset, -1)
        DrawGridLines(vOrigin, vSY, vSX, LY.grid, LY.offset, LX.offset, -1)
    end
    ...
    local function DrawstrsnapCross(vsCenter, r, g, b)
    ...
    local function OnPaintHUD()
        target.active = false
        if GetConVarNumber("strsnap_clickgrid") ~= 0 and not strsnapclick then return end
        strsnapclickprev = strsnapclick
        strsnapclick = strsnapclickprev and strsnapclickfade > CurTime()
        if (GetConVarNumber("strsnap_enabled") == 0) then return end
        ...
        if (target.locked) then
            ...
            if (class ~= 'prop_physics' and class ~= 'phys_magnet' and class ~= 'gmod_spawner' and GetConVarNumber('strsnap_allentities') == 0 or class == 'player') then return end
            ...
            if (LocalPlayer():GetActiveWeapon():GetClass() ~= 'gmod_tool' and GetConVarNumber('strsnap_alltools') == 0) then return end
            target.entity = entity
        end

        --ErrorNoHalt(collectgarbage("count"))
        if GetConVarNumber("strsnap_gcboost") ~= 0 then
            collectgarbage("step", GetConVarNumber("strsnap_gcstrength"))
        end

        strsnapclick = strsnapclickprev
        strsnapclickfade = CurTime() + 0.25
        ...
        if cache.eEntity ~= target.entity or cache.vEntAngles ~= target.entity:GetAngles() or vEntPosition ~= target.entity:GetPos() then
            ...
            local obvstrsnap = ComputeEdges(target.entity, obbmax, obbmin)
            ...
            cache.aGrid = obvgrid
            cache.astrsnap = obvstrsnap
            cache.aFaces = faces
        end

        local obvgrid = cache.aGrid
        local obvstrsnap = cache.astrsnap
        ...
        if (not target.locked) then
            ...
            if (distance > GetConVarNumber("strsnap_distance")) then return end
        ...
        if (GetConVarNumber("strsnap_boundingbox") ~= 0) then
        ...
        if (not target.locked) then
            vectorGrid = vectorOrigin + Calculatestrsnap(vectorX, vectorY, intersection)
            ...
            local vectorstrsnap = trace.HitPos
            target.offset = target.entity:WorldToLocal(vectorstrsnap)
            ...
            elseif (trace.HitPos == trace.StartPos) then
                ...
                strsnaperror = (LocalPlayer():GetEyeTrace().HitPos - trace.HitPos):Length()
                target.error = false

                if ((vectorstrsnap - vectorGrid):Length() > 0.5) then
                    local marker = PointToScreen(vectorstrsnap)

                    if (marker) then
                        DrawstrsnapCross(marker, 255, 255, 255)
                    end
                end
            end
        else
            vectorGrid = target.entity:LocalToWorld(target.vector)
            local vectorstrsnap = target.entity:LocalToWorld(target.offset)
            local marker = PointToScreen(vectorstrsnap)
            strsnaperror = (LocalPlayer():GetEyeTrace().HitPos - vectorstrsnap):Length()

            if (marker) then
                if (target.error == true) then
                    strsnaperror = -2
                    DrawstrsnapCross(marker, 0, 255, 255)
                elseif (strsnaperror < 0.001) then
                    DrawstrsnapCross(marker, 0, 255, 0)
                elseif (strsnaperror < 0.1) then
                    DrawstrsnapCross(marker, 255, 255, 0)
                else
                    DrawstrsnapCross(marker, 255, 0, 0)
                end
            end
        end

        if (GetConVarNumber("strsnap_hidegrid") == 0) then
        ...
        if (vsCursor) then
            if (strsnaperror == -1) then
                target.active = false
                DrawstrsnapCross(vsCursor, 0, 255, 255)
            elseif (strsnaperror == -2) then
                DrawstrsnapCross(vsCursor, 255, 0, 255)
            elseif (strsnaperror < 0.001) then
                DrawstrsnapCross(vsCursor, 0, 255, 0)
            elseif (strsnaperror < 0.1) then
                DrawstrsnapCross(vsCursor, 255, 255, 0)
            else
                DrawstrsnapCross(vsCursor, 255, 0, 0)
            end
        end
    end

    local function OnstrsnapView(player, origin, angles, fov)
        local targetvalid = target.active and target.locked and target.entity:IsValid()
        local strsnaptargetvalid = strsnaptarget.active and strsnaptarget.locked and strsnaptarget.entity:IsValid()

        if (strsnapcursor and not strsnapspawnmenu and targetvalid) then
            local screen = ToScreen(target.entity:LocalToWorld(target.offset))
            gui.SetMousePos(math.Round(screen.x), math.Round(screen.y))
        end

        if (not strsnapcursor and targetvalid) then
            return {
                angles = (target.entity:LocalToWorld(target.offset) - player:GetShootPos()):Angle()
            }
        elseif (strsnaplock and strsnaptargetvalid) then
            return {
                angles = (strsnaptarget.entity:LocalToWorld(strsnaptarget.offset) - player:GetShootPos()):Angle()
            }
        end
    end

    local function OnstrsnapAim(user)
        local targetvalid = target.active and target.locked and target.entity:IsValid()
        local strsnaptargetvalid = strsnaptarget.active and strsnaptarget.locked and strsnaptarget.entity:IsValid()
        ...
        elseif (strsnaplock and strsnaptargetvalid) then
            user:SetViewAngles((strsnaptarget.entity:LocalToWorld(strsnaptarget.offset) - LocalPlayer():GetShootPos()):Angle())
        end
    end

    concommand.Add("+strsnap", strsnapPress)
    concommand.Add("-strsnap", strsnapRelease)
    concommand.Add("strsnaplock", strsnapLock)
    concommand.Add("strsnaptogglegrid", strsnapToggleGrid)
    hook.Add("Initialize", "SmartstrsnapInitialize", OnInitialize)
    hook.Add("SpawnMenuOpen", "SmartstrsnapSpawnMenu", OnSpawnMenu)
    hook.Add("Think", "SmartstrsnapThink", OnThink)
    hook.Add("ShutDown", "SmartstrsnapShutDown", OnShutDown)
    hook.Add("KeyPress", "SmartstrsnapKeyPress", OnKeyPress)
    hook.Add("KeyRelease", "SmartstrsnapKeyRelease", OnKeyRelease)
    hook.Add("CreateMove", "Smartstrsnapstrsnap", OnstrsnapAim)
    hook.Add("CalcView", "SmartstrsnapstrsnapView", OnstrsnapView)
    hook.Add("SpawnMenuOpen", "SmartstrsnapSpawnMenu", OnSpawnMenu)
    hook.Add("HUDPaintBackground", "SmartstrsnapPaintHUD", OnPaintHUD)
    ...
    local function OnPopulateToolPanel(panel)
        ...
        panel:AddControl("CheckBox", {
            Label = "Enable",
            Command = "strsnap_enabled"
        })

        panel:AddControl("CheckBox", {
            Label = "Use click grid (USE temporarily enables grid)",
            Command = "strsnap_clickgrid"
        })

        panel:AddControl("CheckBox", {
            Label = "Hide grid (only shows strsnap point)",
            Command = "strsnap_hidegrid"
        })

        panel:AddControl("CheckBox", {
            Label = "Smart toggle enabled",
            Command = "strsnap_enabletoggle"
        })

        panel:AddControl("CheckBox", {
            Label = "Revert aim to grid strsnap on detach",
            Command = "strsnap_revertaim"
        })

        panel:AddControl("CheckBox", {
            Label = "Enable for all entities",
            Command = "strsnap_allentities"
        })

        panel:AddControl("CheckBox", {
            Label = "Enable for all tools",
            Command = "strsnap_alltools"
        })

        panel:AddControl("CheckBox", {
            Label = "Draw thick center lines",
            Command = "strsnap_centerline"
        })

        panel:AddControl("Slider", {
            Label = "Grid toggle delay (double click strsnap-key)",
            Command = "strsnap_toggledelay",
            Type = "Float",
            Min = "0.0",
            Max = "0.2"
        })
        ...
        panel:AddControl("CheckBox", {
            ...
            Command = "strsnap_boundingbox"
        })

        panel:AddControl("Slider", {
            Label = "Grid draw distance",
            Command = "strsnap_distance",
            Type = "Integer",
            Min = "50",
            Max = "1000"
        })
        ...
        panel:AddControl("Slider", {
            Label = "Maximum number of strsnap points on an axis",
            Command = "strsnap_gridlimit",
            Type = "Integer",
            Min = "2",
            Max = "64"
        })
        ...
        panel:AddControl("Slider", {
            Label = "Minimum distance between each strsnap point",
            Command = "strsnap_gridsize",
            Type = "Integer",
            Min = "2",
            Max = "64"
        })
        ...
        panel:AddControl("CheckBox", {
            ...
            Command = "strsnap_gcboost"
        })
    end

    function OnPopulateToolMenu()
        spawnmenu.AddToolMenuOption("Options", "Player", "SmartstrsnapSettings", "Smartstrsnap", "", "", OnPopulateToolPanel, {
            SwitchConVar = 'strsnap_enabled'
        })
    end

    hook.Add("PopulateToolMenu", "SmartstrsnapToolMenu", OnPopulateToolMenu)
end