...
local font_prefix = "SH_POINTSHOP."
...
local function L(...) return base_table:L(...) end
...
function base_table:MakeWindow(title, border)
	...
	pnl.Paint = function(me, w, h)
		...
		if (self.BlurredBackgrounds) then
			self:DrawBlur(me, 2)
		end
	...
	pnl.CloseTemporary = function(me, wnd)
		wnd:SetAlpha(0)
		wnd:SetVisible(false)
		wnd.OnRemove = function()
			me:SetVisible(true)
			me:AlphaTo(255, 0.1)
		end

		me:AlphaTo(0, 0.1, nil, function()
			me:SetVisible(false)
...
function base_table:QuickEntry(tx, parent, alt)
	...
	entry:SetText(tx or "")
	self:EntryStyle(entry, alt)

	return entry
end

function base_table:EntryStyle(entry, alt)
	local styl = self.Style
	...
	entry.Paint = function(me, w, h)
		...
		if (me.m_Placeholder and me:GetText() == "" and !me:HasFocus()) then
			draw.SimpleText(me.m_Placeholder .. ".. ", me:GetFont(), 5, h * 0.5, me:GetTextColor(), TEXT_ALIGN_LEFT, TEXT_ALIGN_CENTER)
		end
	end
end

function base_table:QuickComboBox(parent, alt)
	parent:SetMouseInputEnabled(true)

	local styl = self.Style
	local bg = alt and styl.bg or styl.inbg

	local combo = vgui.Create("DComboBox", parent)
	combo.m_bNoHover = true
	combo.m_Background = bg
...
function base_table:PaintScroll(panel, width)
	local styl = self.Style
	...
	if (panel.Rebuild) then
		panel.PerformLayout = function(me)
			...
			if (me.VBar.Enabled) then Wide = Wide - (width or (me.VBar:GetWide() - m * 0.5)) end
			...
			if (width) then
				me.VBar:SetWide(width)
				me.VBar:AlignRight(0)
			end
		end
	end
	...
	scr.Paint = function(_, w, h)
		draw.RoundedBox(4, 0, 0, w, h, /* 76561198006360138 styl.header */ styl.bg)
	end
...
// https://facepunch.com/showthread.php?t=1522945&p=50524545&viewfull=1#post50524545|76561198006360138
...
function base_table:Avatar(ply, siz, par)
	...
	pnl.Paint = function() end
		...
		av:SetSize(siz, siz)
			...
			btn:SetTooltipLounge("Click here to view " .. (isstring(ply) and "their" or ply:Nick() .. "'s") .. " Steam Profile")
			btn:SetText("")
			btn:Dock(FILL)
...
function base_table:NavBar(parent, tabs, selectable, iconless)
	...
	navbar.PreToggle = function(me, b) end
	navbar.OnToggle = function(me, b) end

		local scroll = vgui.Create("DScrollPanel", navbar)
		scroll:Dock(FILL)
		self:PaintScroll(scroll, 1)

		local togglenavbar
		if (!iconless) then
			togglenavbar = vgui.Create("DButton", navbar)
			togglenavbar:SetText("")
			togglenavbar:SetTooltipLounge(self.Language.toggle)
			...
			togglenavbar.DoClick = function(me, btoggle)
				if (btoggle ~= nil) then
					if (btoggle == toggled) then
						return end
				...
				navbar:PreToggle(toggled)

				_TAB_LOUNGE_RESIZING = true
				navbar:Stop()
				navbar:SizeTo(toggled and th or th * 3.25, -1, 0.1, 0, 0.2, function()
					_TAB_LOUNGE_RESIZING = false
					navbar:OnToggle(toggled)
				end)

				for _, v in pairs (tabs) do
					if (IsValid(v.m_Children)) then
						if (toggled) then
							v.m_Button.m_bWasToggled = v.m_Button.m_bToggle
							v.m_Button:ToggleChildren(false)
						elseif (v.m_Button.m_bWasToggled or selc) then
							v.m_Button:ToggleChildren(true)
						end
					end
				end
			end
		...
		local elements = {}
		navbar.RebuildTabs = function(me, tabs)
			local pagepnls = {}
			local order = 0
			...
			for i, page in ipairs (tabs) do
				...
				btn:SetText("")
				btn:SetZPos(order)
				btn:SetTooltipLounge(page.desc or tx)
				btn:SetTall(th)
				if (page.dock) then
					btn:Dock(page.dock)
				else
					btn:Dock(TOP)
					btn:SetParent(scroll)
				end
				btn.Paint = function(me, w, h)
					if (me.Hovered) then
					...
					if (page.icon) then
						...
						surface.DrawTexturedRectRotated(24 * scale, 24 * scale, 32 * scale, 32 * scale, 0)
					end
				...
				btn.DoClick = function()
					...
					if (func and func(unpack(args)) == false) then
						sel = nil
						selc = nil
						return
					end
				...
				order = order + (page.children and 10 or 1)

				-- Subpages
				if (page.children) then
					local chi = vgui.Create("DPanel", navbar)
					chi:SetZPos(btn:GetZPos() + 1)
					chi:SetVisible(iconless == true)
					chi:SetDrawBackground(false)
					chi:SetTall(1)
					if (page.dock) then
						chi:Dock(page.dock)
					else
						chi:Dock(TOP)
						chi:SetParent(scroll)
					end
					page.m_Children = chi
					table.insert(elements, chi)

						local chih = 0
						for j, ch in pairs (page.children) do
							local cbtn = vgui.Create("DButton", chi)
							cbtn:SetText("")
							cbtn:SetTooltipLounge(ch.desc and L(ch.desc) or nil)
							cbtn:SetTall(th * 0.5)
							cbtn:Dock(TOP)
							cbtn.Paint = function(me, w, h)
								if (me.Hovered) then
									surface.SetDrawColor(self.Style.hover)
									surface.DrawRect(0, 0, w, h)
								end
								...
								if (sel == i and selc == j) then
									surface.SetDrawColor(self.Style.header)
									surface.DrawRect(0, 0, th * 0.1, h)
								end
							end
							cbtn.DoClick = function(me)
								sel = i
								selc = j
								...
								local lbl = self:QuickLabel("• " .. L(ch.text), font_prefix .. "Medium", self.Style.text, cbtn)
								lbl:Dock(LEFT)
								lbl:DockMargin(m, 0, 0, 0)

							chih = chih + cbtn:GetTall()
						end

					if (!iconless) then
						btn.m_bToggle = false
						btn.ToggleChildren = function(me, b)
							if (b) then
								togglenavbar:DoClick(false)
							end

							if (b ~= nil) then
								if (b == me.m_bToggle) then
									return end

								me.m_bToggle = b
							else
								me.m_bToggle = !me.m_bToggle
							end

							chi:Stop()
							chi:SetVisible(true)
							chi:SizeTo(-1, me.m_bToggle and chih or 1, 0.1, nil, nil, function()
								chi:SetVisible(me.m_bToggle == true)
							end)
						end
						btn.DoClick = function(me)
							if (me.m_bToggle and selc and sel == i) then
								return end

							me:ToggleChildren(!me.m_bToggle)
						end
					end
				end
			end
		...
		navbar:RebuildTabs(tabs)

	if (!iconless) then
		navbar.Think = function(me)
			if (input.IsKeyDown(KEY_TAB)) then
				if (!me.m_bHoldingTab) then
					local kbf = vgui.GetKeyboardFocus()
					if not (IsValid(kbf) and kbf:GetClassName() == "TextEntry") then
...
function base_table:CreateGroup(text, parent, ...)
	...
	p.Paint = function(me, w, h)
		...
		local lbl
		p.SetLabel = function(me, tx)
			if (!lbl) then
				if (tx ~= "") then
					lbl = self:QuickLabel(tx, font_prefix .. "Large", self.Style.text, p)
					lbl:Dock(TOP)
...
local matChecked = Material("shenesis/general/checked.png", "noclamp smooth")
...
function base_table:CheckboxLabel(text, cvar, parent)
	...
	pnl:SetTall(20)
		...
		local lbl = self:QuickLabel(text, font_prefix .. "Medium", self.Style.text, pnl)
		lbl:Dock(FILL)
		...
		chk.Paint = function(me, w, h)
			...
			elseif (pnl.m_bDrawBackground) then
...
function base_table:InstallModelControls(p)
	p.m_iX = 0
	p.m_iY = 0
	...
	p.OnMousePressed = function(me, mc)
		if (mc == MOUSE_LEFT) then
			me.m_bDragging = true
			me.m_iX = gui.MouseX()
	...
	p.OnMouseReleased = function(me, mc)
		if (mc == MOUSE_LEFT) then
			me.m_bDragging = false
		elseif (mc == MOUSE_RIGHT) then
			me.m_bZooming = false
...
function base_table:NumSlider(parent)
	local styl = self.Style

	local slider = vgui.Create("DNumSlider", parent)
	slider.TextArea:SetTextColor(styl.text)
	slider.TextArea:SetFont(font_prefix .. "Medium")
	slider.TextArea:SetDrawLanguageID(false)
	slider.TextArea:SetCursorColor(styl.text)
	...
	slider.Slider.Knob.Paint = function(me, w, h)
		draw.RoundedBox(4, 0, 0, w, h, styl.header)
...
function base_table:DrawBlur(me, intensity)
	...
	for i = 1, 1, 0.33 do
		matBlurScreen:SetFloat("$blur", math.Clamp(intensity, 0, 10) * i)
