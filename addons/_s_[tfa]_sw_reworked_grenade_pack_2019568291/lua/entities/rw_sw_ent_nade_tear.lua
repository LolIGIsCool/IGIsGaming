...
function ENT:Think()
    ...
    for k, v in pairs( ents.FindInSphere( self:GetPos(), 216 ) ) do
            if v:GetModel() == "models/banks/ig/imperial/riot/riot_commander/riot_commander.mdl" then continue end
            if v:GetModel() == "models/banks/ig/imperial/riot/riot_trooper/riot_trooper.mdl" then continue end
            
            if (v:IsPlayer() or v:IsNPC()) then
                damage = DamageInfo()
                damage:SetDamage( math.random( 1, 2 ) )
...
function ENT:Detonate(self,pos)
	...
	if SERVER then
		...
		util.Effect("tfa_vanilla_tear", gas)
	end
