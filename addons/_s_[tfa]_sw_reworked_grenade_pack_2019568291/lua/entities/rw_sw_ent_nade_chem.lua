...
function ENT:Think()
    ...
    for k, v in pairs( ents.FindInSphere( self:GetPos(), 216 ) ) do
            if ((v:IsPlayer() or v:IsNPC()) && v:GetModel() ~= "models/player/hydro/swbf_bossk/swbf_bossk.mdl") then
                damage = DamageInfo()
                damage:SetDamage( math.random( 11, 15 ) )
...
function ENT:Detonate(self,pos)
	...
	if SERVER then
		...
		util.Effect("tfa_vanilla_chem", gas)
	end
