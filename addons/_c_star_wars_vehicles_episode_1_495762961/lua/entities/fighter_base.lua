...
if CLIENT then
--CreateConVar( "moose_muteships1", "0", FCVAR_USERINFO )
CreateClientConVar("moose_muteships",0,true,false)
end
...
function ENT:Enter(p)
	...
	if(!self.Inflight and !self.Done) then
		...
		local shipseat = "pilot"
		local MoosePlayerEnterShipPilot = hook.Call( "MoosePlayerEnterShip", GAMEMODE, p, self.Vehicle, shipseat)
	end
end
...
function ENT:Exit(kill) --####### Get out @RononDex
	...
	if (IsValid(self.Pilot)) then
		...
		self.Pilot:Spawn();
		self.Pilot:SetNWString("LastVech",self.Vehicle)
	...
	if(IsValid(v) and IsValid(p)) then
        ...
        if(e.IsSWVehicle) then
            if(v["Is" .. e.Vehicle .. "Seat"]) then
				...
				shipseat = "passenger"
				local MoosePlayerEnterShipPilot = hook.Call( "MoosePlayerEnterShip", GAMEMODE, p, e:GetClass(), shipseat)
            end
		end
	end
end);
	...
	if(self.Inflight) then
		...
		if(IsValid(self.Pilot)) then
            ...
            if(self.HasLightspeed) then
                ...
                --if( self.Pilot:SteamID() == "STEAM_0:1:102265670" or self.Pilot:SteamID() == "STEAM_0:0:52423713" ) then
				if self:GetClass() == "krennic" or self.Pilot:SteamID() == "STEAM_0:0:52423713" then
                ...
                if(self.LightSpeed) then
                    if(self.DistanceMode) then
                        ...
                        self:PunchingIt(Vector(-14838,11576,13800));
                    end
                end
            end
...
function ENT:PhysicsCollide(cdat, phys)
	...
	if(self.LastCollide < CurTime() and !self.Land and !self.TakeOff and Should_Collisions) then
		if cdat.HitEntity:GetClass() == "player" then return end
...
if CLIENT then
	...
	function ENT:Think()
		...
		if(Flying) then
			...
			--if GetConVar("moose_muteships1"):GetInt() == 0 then
				
				
			--if p:SteamID() ~= "STEAM_0:0:52423713" then	
