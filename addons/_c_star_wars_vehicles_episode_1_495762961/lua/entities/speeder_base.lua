		...
		if(IsValid(speeder) and speeder.IsSWVehicle) then
			if(e.IsSpeederChair) then
				if(!IsValid(speeder.Pilot) and !speeder.Inflight) then
					...
					shipseat = "pilot"
					local MoosePlayerEnterShipPilot = hook.Call( "MoosePlayerEnterShip", GAMEMODE, p, speeder:GetClass(), shipseat)
				end
			elseif(e.SpeederPassenger) then
				if(!IsValid(speeder.Passenger)) then
					speeder:Enter(p,false);
					shipseat = "passenger"
					local MoosePlayerEnterShipPassenger = hook.Call( "MoosePlayerEnterShip", GAMEMODE, p, speeder:GetClass(), shipseat)
				end
			end
		end
	end
...
local function GetVehicleHealthKumo(e)
...
local function AlertBreakdown(ply,typer)
	if SERVER then
		if not ply or not ply:IsPlayer() then return end
		if ply.alertedbreakdown then return end
		ply.alertedbreakdown = true
		if typer == "dmg" then
			ply:ChatPrint("Your vehicle has broken down as it has been damaged too much, see an engineer for repairs")
		elseif typer == "rand" then
			ply:ChatPrint("Your vehicle had a malfunction and it has stopped working, see an engineer for repairs")
		end
		timer.Create("breakdownalertcd"..ply:SteamID64(),5,1,function() ply.alertedbreakdown = false end)
	end
end

local function VehcCanFly(vehc, healthpercent)
	if vehc.Tractored then return false end
	if healthpercent <= 20 then AlertBreakdown(vehc.Pilot,"dmg") return false end
	if vehc.brokendown then AlertBreakdown(vehc.Pilot,"rand") return false end
	return true
end
...
function ENT:PhysicsSimulate( phys, deltatime )
	...
	local worldZ;
	self:RunTraces();
	local health,starthealth = GetVehicleHealthKumo(self)
	local healthpercent = math.Round(health/starthealth * 100)
 	if VehcCanFly(self, healthpercent) then
...
if CLIENT then
	...
	function ENT:Think()
		...
		if(SpeederActive) then
			...
			if cvars.Bool("moose_muteships") then
				--print("ayylmao")
			else
