...
function ENT:EngineActiveChanged( bActive )
	if bActive then
		self.ENG = CreateSound( self, "AWING_ENGINE" )
		self.ENG:PlayEx(0,0)
		
		self.DIST = CreateSound( self, "AWING_DIST" )
...
function ENT:Draw()
	...
	local Size = 100 + (self:GetRPM() / self:GetLimitRPM()) * 40 + Boost

	render.SetMaterial( mat )
	render.DrawSprite( self:LocalToWorld( Vector(-110,-50,0) ), Size, Size, Color( 255, 240, 150, 255) )
	render.DrawSprite( self:LocalToWorld( Vector(-110,50,0) ), Size, Size, Color( 255, 240, 150, 255) )
