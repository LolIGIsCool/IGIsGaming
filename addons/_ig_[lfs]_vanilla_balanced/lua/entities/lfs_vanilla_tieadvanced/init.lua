...
function ENT:SpawnFunction( ply, tr, ClassName ) -- called by garry
	...
	ent:SetPos( tr.HitPos + tr.HitNormal * 90 ) -- spawn x units above ground
	ent:Spawn()
	ent:Activate()
...
function ENT:PrimaryAttack()
	...
	local fP = { Vector(49.91,14.53,-42.09),Vector(49.91,-14.53,-42.09) } --  -y,x,z
...
function ENT:SecondaryAttack()
	...
	local rocketpos = {
		self:GetPos()+self:GetForward()*17+self:GetUp()*1+self:GetRight()*78.22,
		self:GetPos()+self:GetForward()*17+self:GetUp()*1+self:GetRight()*-78.22,
	}
	...
	if (self:GetAmmoSecondary()+1)%2 == 0 then
		mPos = rocketpos[1]
