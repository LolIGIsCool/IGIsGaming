...
local function voteDone( t )
	...
	local winner
	local winnernum = 0
	local player_votes = 0
	ULib.tsay( _, "[ VOTE RESULTS ]" ) -- TODO, color?
	for id, numvotes in pairs( results ) do
		ULib.tsay( _, "Option [ " .. t.options[ id ] .. " ] votes recieved (" .. numvotes .. ")" ) -- TODO, color?
		player_votes = player_votes + numvotes
	...
	if not winner then
		...
		str = "[ VOTE FINAL RESULT ] Option [ " .. t.options[ winner ] .. " ] won with (" .. winnernum .. "/" .. player_votes .. ") votes!"
	end
	ULib.tsay( _, str ) -- TODO, color?
	ulx.logString( str )
	Msg( str .. "\n" )
