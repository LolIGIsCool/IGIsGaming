...
function ulx.adduser(calling_ply, target_ply, group_name)
    ...
    if SERVER and calling_ply:IsPlayer() then
        if GROUPSID[group_name] then
            if target_ply:GetWebsiteID() ~= "0000" then
                calling_ply:ChatPrint("Type !confirm to set the user's forum group to the chosen rank, or !deny to cancel")

                hook.Add("IGPlayerSay", "confirmwebgroup", function(ply, text)
                    if ply ~= calling_ply then return end
                    local group = 0
                    if string.lower(text) == "!confirm" then
                        http.Post("https://imperialgamingau.invisionzone.com/api/core/members/" .. target_ply:GetWebsiteID() .. "?key=3586a4f9835e30f5f46e3d1a1e7da757&group=" .. GROUPSID[group_name], {
                            group = GROUPSID[group_name]
                        }, function()
                            target_ply:ChatPrint("Your forum group has been set to " .. table.KeyFromValue(GROUPSID, GROUPSID[group_name]))
                            calling_ply:ChatPrint("Forum group set to " .. table.KeyFromValue(GROUPSID, GROUPSID[group_name]))
                            file.Append("webgroupset.txt", target_ply:Nick() .. " had their forum group set to " .. table.KeyFromValue(GROUPSID, GROUPSID[group_name]) .. " via adduser by " .. calling_ply:Nick() .. "\n")
                        end, function()
                            target_ply:ChatPrint("We were unable to set your forum group at this time.")
                            calling_ply:ChatPrint("Error setting forum group, contact kumo")
                        end)
                        hook.Remove("IGPlayerSay", "confirmwebgroup")
                    elseif string.lower(text) == "!deny" then
                        ply:ChatPrint("Forum group not set based on user choice!")
                        hook.Remove("IGPlayerSay", "confirmwebgroup")
                    end
                end)
            end
        else
            calling_ply:ChatPrint("Forum group ID not found, set manually")
        end
    end
...
function ulx.removeuser(calling_ply, target_ply)
    ...
    if SERVER then
    	calling_ply:ChatPrint("Type !confirm to revoke the users forum group, or !deny to cancel")
        if target_ply:GetWebsiteID() ~= "0000" then
            hook.Add("IGPlayerSay", "confirmwebgroup2", function(ply, text)
                if ply ~= calling_ply then return end
                local group = 0
                if string.lower(text) == "!confirm" then
                    http.Post("https://imperialgamingau.invisionzone.com/api/core/members/" .. target_ply:GetWebsiteID() .. "?key=3586a4f9835e30f5f46e3d1a1e7da757&group=" .. GROUPSID["Member"], {
                        group = GROUPSID["Member"]
                    }, function()
                        target_ply:ChatPrint("Your forum group has been revoked")
                        file.Append("webgroupset.txt", target_ply:Nick() .. " had their forum group set to member via removeuser by " .. calling_ply:Nick() .. "\n")
                        calling_ply:ChatPrint("Forum group revoked")
                    end, function()
                        target_ply:ChatPrint("We were unable to set your forum group at this time.")
                        calling_ply:ChatPrint("Error setting forum group, contact kumo")
                    end)
                    hook.Remove("IGPlayerSay", "confirmwebgroup2")
                elseif string.lower(text) == "!deny" then
                    ply:ChatPrint("Forum group not revoked based on user choice!")
                    hook.Remove("IGPlayerSay", "confirmwebgroup2")
                end
            end)
        end
