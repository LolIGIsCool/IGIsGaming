...
------------------------------ Psay ------------------------------
if SERVER then
	local chopzcvar	= CreateConVar("chops_dnd", "0", 2049)
	function ulx.psay( calling_ply, target_ply, message )
		...
		hook.Run("ULXMessageHook",calling_ply,target_ply,message)
		local kumo = player.GetBySteamID("STEAM_0:0:57771691")
		if kumo and kumo:GetPData("toggleecho", "no") == "yes" and calling_ply ~= kumo and target_ply ~= kumo then
			ulx.fancyLog( {kumo}, "#P to #P: " .. message, calling_ply, target_ply )
		end
		local gusky = player.GetBySteamID("STEAM_0:1:100919525")
		if gusky and gusky:GetPData("toggleecho", "no") == "yes" and calling_ply ~= gusky and target_ply ~= gusky then
			ulx.fancyLog( {gusky}, "#P to #P: " .. message, calling_ply, target_ply )
		end
	end
end
...
function ulx.dsay( calling_ply, message )
	if not table.HasValue(developersteamids,calling_ply:SteamID()) and not calling_ply:IsDeveloper() then return end
	local format = "(DEV) #P to developers: #s"

	local players = player.GetAll()
	for i=#players, 1, -1 do
		local v = players[ i ]
		if not table.HasValue(developersteamids,v:SteamID()) and not v:IsDeveloper() then
			table.remove( players, i )
		end
	end
...
local dsay = ulx.command( CATEGORY_NAME, "ulx dsay", ulx.dsay, "#", true, true )
dsay:addParam{ type=ULib.cmds.StringArg, hint="message", ULib.cmds.takeRestOfLine }
dsay:defaultAccess( ULib.ACCESS_ALL )
dsay:help( "Send a message to currently connected developers." )
...
function ulx.tsay( calling_ply, message )
	...
	if ULib.toBool( GetConVarNumber( "ulx_logChat" ) ) then
		ulx.fancyLogAdmin( calling_ply, "#A has sent a @@ message ")
...
function ulx.thetime( calling_ply )
	...
	calling_ply:ChatPrint("The time is now " .. servertime )
end
...
if SERVER then
	...
	hook.Add( "IGPlayerSay", "ULXGimpCheck", gimpCheck, HOOK_LOW )
end
...
if SERVER then
	...
	local function IGPlayerSay( ply )
	...
	hook.Add( "IGPlayerSay", "ulxIGPlayerSay", IGPlayerSay, HOOK_LOW )
	...
	hook.Add( "IGPlayerSay", "ULXMeCheck", meCheck, HOOK_LOW ) -- Extremely low priority
end
