...
function ulx.map(calling_ply, map, gamemode)
    local fuck123 = {
        ...
        ["success"] = function(lol1, lol2, lol3)
            local gottable = util.JSONToTable(lol2)["attributes"]
            local startup = gottable["container"]["startup_command"]
            local apienvironment = gottable["container"]["environment"]
            apienvironment["SRCDS_MAP"] = map
            local egg = gottable["egg"]
            local image = gottable["container"]["image"]
            ...
            local patchrequest = util.TableToJSON(bumtable)

            local fuck1234 = {
                ["failed"] = function() end,
                ["success"] = function(lol12, lol22, lol32) end,
                ["method"] = "PATCH",
                ["url"] = "https://panel.imperialgaming.net/api/application/servers/1/startup",
                ["headers"] = {
                    ["Authorization"] = "Bearer mNWToVTjjR5YmE6IeMNRGTxUz5N2VcxG08URnadt4mzKlyBy",
        ...
        ["method"] = "GET",
        ["url"] = "https://panel.imperialgaming.net/api/application/servers/1",
        ["headers"] = {
            ["Authorization"] = "Bearer mNWToVTjjR5YmE6IeMNRGTxUz5N2VcxG08URnadt4mzKlyBy",
            ["Content-Type"] = "application/json",
            ["Accept"] = "Application/vnd.pterodactyl.v1+json"
	...
	for i=1, #target_plys do
		...
		if v.NoNoclip then
			...
			if v:GetMoveType() == MOVETYPE_WALK then
				...
				v:SetNoDraw(true)
				if not v.StickNoTarget then
					v:SetNoTarget(true)
				end
				v:GodEnable()
				v:SetNWBool("staffnoclip",true)
				v.ULXHasGod = true
				ulx.fancyLogAdmin( calling_ply, true, "#A noclipped #T", target_plys, v )
...
------------------------------ IG Noclip ------------------------------
function ulx.ignoclip( calling_ply, target_plys )
	if not target_plys[ 1 ]:IsValid() then
		Msg( "You are god, you are not constrained by walls built by mere mortals.\n" )
		return
	end
...
local ignoclip = ulx.command( CATEGORY_NAME, "ulx ignoclip", ulx.ignoclip, "!ignoclip" )
ignoclip:addParam{ type=ULib.cmds.PlayersArg, ULib.cmds.optional }
ignoclip:defaultAccess( ULib.ACCESS_ADMIN )
ignoclip:help( "Toggles ignoclip on target(s)." )
