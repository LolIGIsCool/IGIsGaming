...
if(CLIENT) then
	...
	net.Receive("sv_npctool_spawner_spawn",function(len,pl)
		if not pl:IsAdmin() and not pl:HasWeapon("gmod_tool") then gBan:PlayerBanID( nil, pl:SteamID(), 0, "Automatically banned for attempting to exploit.", false ) return end
		local pos = net.ReadVector()
		local yaw = net.ReadFloat()
