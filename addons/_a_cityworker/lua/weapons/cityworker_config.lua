...
if CLIENT then
    ...
    function SWEP:DrawHUD()
        local count = 0
        for k, v in pairs( GLOBALPPTABLELOL.PropsList ) do
            local scrPos = v.Pos:ToScreen()
            if LocalPlayer():GetPos():DistToSqr(v.Pos) >= 1000000 then continue end
            count = count + 1
            draw.SimpleTextOutlined( "ID: "..k.." ("..v.Model..")", "Trebuchet18", scrPos.x, scrPos.y - 15, Color( 255, 255, 255 ), TEXT_ALIGN_CENTER, TEXT_ALIGN_TOP, 1, Color( 0, 0, 0 ) )
        ...
        chat.AddText("[PP] Rendering "..count.." PermaProps")
    end
