...
function SWEP:Think()
	...
	elseif self.Owner:KeyReleased(IN_ATTACK) and self.Charge>0 then
        self.Owner:SetNWBool("ImperialArtsCanSwitch", false);

        timer.Simple(1,function()
            self.Owner:SetNWBool("ImperialArtsCanSwitch", true);
        end)
	...
	elseif self.Owner:KeyDown(IN_WALK) then --Throwing
		--if self.Owner:GetNWInt( 'MeleeArts2Stamina' )<=self.ThrowStamina then return end
		if self.CanThrow == false or GetConVarNumber( "ma2_togglethrowing" ) == 0 then return end
