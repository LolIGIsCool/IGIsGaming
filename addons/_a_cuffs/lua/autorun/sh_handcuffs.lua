...
hook.Add( "SetupMove", "Cuffs Move Penalty", function(ply, mv, cmd)
	...
	if cuffs:GetRopeLength() ~= 201 then
	...
	local distFromTarget = ShootPos:DistToSqr( TargetPoint )
	...
	local TargetPos = TargetPoint - (MoveDir)
	...
	local accel = 400 -- Change this if they can resist being pulled -Moose
...
if CLIENT then
	...
	local function UpdateCuffsTable()
		...
		for _,v in pairs(ents.FindByClass("weapon_handcuffed")) do
	...
	hook.Add( "PostDrawOpaqueRenderables", "Cuffs DragRope", function()
		...
		for i=1,#allCuffs do
			...
			local kidPos = (kidnapper:IsPlayer() and kidnapper:GetPos() + vector1) or kidnapper:GetPos()
