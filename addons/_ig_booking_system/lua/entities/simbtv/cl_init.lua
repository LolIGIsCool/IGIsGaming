...
function ENT:Draw()
	...
	local simbbooking = GetGlobalInt("simbbooking")
	local scale2 = 0.4
	if string.len( simbbooking ) <= 16 then
		scale2 = 0.4
    elseif string.len( simbbooking ) >= 16 then
    	scale2 = 0.3
    else 
        scale2 = 0.4
	end
	...
	cam.Start3D2D(self:LocalToWorld(d1), self:LocalToWorldAngles(d11), scale)
		draw.DrawText("Training Hangar Besh - Booked By", "Trebuchet24", 0,0, Color(255,255,255), TEXT_ALIGN_CENTER)
	cam.End3D2D()
	...
	cam.Start3D2D(self:LocalToWorld(d3), self:LocalToWorldAngles(d11), scale2)
		draw.DrawText(simbbooking, "Trebuchet24", 0,0, Color(255,255,255), TEXT_ALIGN_CENTER)
	cam.End3D2D()
