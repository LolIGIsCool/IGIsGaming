...
function ENT:Draw()
	...
	local scale2 = 0.4
	if string.len( mh1booking ) <= 16 then
		scale2 = 0.4
    elseif string.len( mh1booking ) >= 16 then
    	scale2 = 0.3
    else 
        scale2 = 0.4
	end
	...
	cam.Start3D2D(self:LocalToWorld(d1), self:LocalToWorldAngles(d11), scale)
		draw.DrawText("Main Hangar 1 - Booked By", "Trebuchet24", 0,0, Color(255,255,255), TEXT_ALIGN_CENTER)
	cam.End3D2D()
	...
	cam.Start3D2D(self:LocalToWorld(d3), self:LocalToWorldAngles(d11), scale2)
		draw.DrawText(mh1booking, "Trebuchet24", 0,0, Color(255,255,255), TEXT_ALIGN_CENTER)
	cam.End3D2D()
