...
function ENT:Draw()
	...
	local tiehbbooking = GetGlobalInt("tiehbbooking")
	local scale2 = 0.4
	if string.len( tiehbbooking ) <= 16 then
		scale2 = 0.4
    elseif string.len( tiehbbooking ) >= 16 then
    	scale2 = 0.3
    else 
        scale2 = 0.4
	end
	...
	cam.Start3D2D(self:LocalToWorld(d1), self:LocalToWorldAngles(d11), scale)
		draw.DrawText("Simulation Field - Booked By", "Trebuchet24", 0,0, Color(255,255,255), TEXT_ALIGN_CENTER)
	cam.End3D2D()
		cam.Start3D2D(self:LocalToWorld(d2), self:LocalToWorldAngles(d11), scale)
		draw.DrawText("________________________", "Trebuchet24", 0,0, Color(255,255,255), TEXT_ALIGN_CENTER)
	cam.End3D2D()
	cam.Start3D2D(self:LocalToWorld(d3), self:LocalToWorldAngles(d11), scale2)
		draw.DrawText(tiehbbooking, "Trebuchet24", 0,0, Color(255,255,255), TEXT_ALIGN_CENTER)
	cam.End3D2D()
