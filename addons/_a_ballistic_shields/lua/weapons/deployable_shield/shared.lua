...
local function bsGetHoldType(ply)
	if (ply:LookupAttachment( "anim_attachment_RH" )>5) then return {1,"anim_attachment_RH"} end
	if (ply:LookupAttachment( "forward" )>5) then return {2,"forward"} end
...
hook.Add("PlayerDeath","PlayerDeathDeleteShields",function(ply)
	for k,v in pairs(ents.FindByClass("bs_shield")) do
