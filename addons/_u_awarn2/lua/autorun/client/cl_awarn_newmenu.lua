...
function PANEL:Paint()
	...
	surface.SetTextPos( 300 , self:GetTall() - 30 )
	surface.DrawText( "Show all players: " )
...
function PANEL:DrawFrame()
	...
	local DermaCheckbox = vgui.Create( "DCheckBox", self )// Create the checkbox
	DermaCheckbox:SetPos( 425,  self:GetTall() - 28 )
	DermaCheckbox:SetValue( 0 )
	DermaCheckbox.OnChange = function(val)
		if DermaCheckbox:GetChecked() then
			self.OfflinePlayerList:Clear()
			self.titletext = "Offline Players - Showing All Players"
			net.Start("awarn_fetchofflineplayers")
			net.WriteBool(true)
			net.SendToServer()
		else
