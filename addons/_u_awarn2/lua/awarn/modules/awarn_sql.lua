...
function awarn_sendofflineplayers( ply, all )
	...
	print( all )
	if all == true then
		query = "SELECT * FROM awarn_offlinedata ORDER BY lastplayed DESC"
	else
		query = "SELECT * FROM awarn_offlinedata where warnings > 0 ORDER BY lastplayed DESC"
	end
	...
	MySQLite_AWarn.query( query,
		function( result, last )
			...
			if not result then
				...
				ply.totalwarnings = wnum
				local t1 = { Color(60,60,60), "[", Color(30,90,150), "AWarn", Color(60,60,60), "] ", Color(255,255,255), ply:Nick(), " " .. AWarn.localizations[loc].sql10 .. ". " .. AWarn.localizations[loc].sql11 .. ": ", Color(255,0,0), tostring(wnum) }
