...
local blacklist = { ["gmod_wire_keyboard"] = true,
					["uno_table"] = true,
					["bkeycard_pickup"] = true,
					["bkeypad"] = true,
					["ammo_chargepack"] = true,
					["uno_seat"] = true }
...
function META.Sit(ply, EyeTrace, ang, parent, parentbone, func, exit)
	...
	if(not EyeTrace.HitWorld and blacklist[string.lower(EyeTrace.Entity:GetClass())]) then ply:ChatPrint("You cannot sit on this object") return end
	if(not EyeTrace.HitWorld and EyeTrace.Entity:GetModel() and model_blacklist[string.lower(EyeTrace.Entity:GetModel())]) then ply:ChatPrint("You cannot sit on this object") return end
...
local function sitcmd(ply)
	...
	if ply.NextUse and ply.NextUse > now then return end
...
hook.Add("CanExitVehicle","Remove_Seat",function(self, ply)
	...
	if ply.NextUse and CurTime() < ply.NextUse then return false end
