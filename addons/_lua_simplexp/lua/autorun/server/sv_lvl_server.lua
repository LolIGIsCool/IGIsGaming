...
function SimpleXPPlayerSpawn(ply)
	SimpleXPSetXP(ply,SimpleXPLoadData(ply))
end
...
function SimpleXPAddXPSafe(ply,xp)
	if DEBUGMODE then print("Sending XP...") end
	xp = math.floor(xp)
	if not SimpleXPGetXP(ply) then SimpleXPSetXP(ply,SimpleXPLoadData(ply)) end
	...
	SimpleXPAddXP(ply,xp)
end

function SimpleXPAddXP(ply,xp)
	if DEBUGMODE then print("Adding XP...") end
	if _G.HasAugment(ply,"Experience Accumulator I") then
		xp = xp + (xp * 0.05)
	end
	if _G.HasAugment(ply,"Experience Accumulator II") then
		xp = xp + (xp * 0.05)
	end
    if ply:IsUserGroup("patron") then
		xp = xp + (xp * 1)
	end
	xp = math.floor(xp)
	SimpleXPSetXP(ply,SimpleXPGetXP(ply) + xp)
end

function SimpleXPSetXP(ply,xp)
...
function SimpleXPAddXPText(ply,xp,text,override)
...
function SimpleXPTrackTime()
	if NextSpew <= RealTime() then
		for k,v in pairs(player.GetAll()) do
			local xptogive = SimpleXPConfig.GiveXPOnTimeAmount
			if v:GetNWInt("igprogressp", 0) >= 5 then
				xptogive = xptogive * 1.1
			elseif v:GetNWInt("igprogressp", 0) >= 1 then
				xptogive = xptogive * 1.05
			end
			SimpleXPAddXPText(v,xptogive,SimpleXPConfig.GiveXPOnTimeMessage)
		end
...
function SimpleXPHandleMessageAndXP(message,xptoadd,messagetable,xp,override)

	if DEBUGMODE then print("Handling Messages and XP...") end

	if not override and xptoadd > 0 then
	...
	table.insert(messagetable,message .. ": " .. xptoadd .. "XP")
