function ulx.setxp( calling_ply, target_ply, xp, reason )
	...
	for k,v in pairs(target_ply) do
		SimpleXPSetXPText(v,xp,string.upper(reason))
	end
...
function ulx.addxp( calling_ply, target_ply, xp, reason )
	...
	for k,v in pairs(target_ply) do
		SimpleXPAddXPText(v,xp,string.upper(reason),true)
	end
...
function ulx.setlevel( calling_ply, target_ply, level, reason )
	...
	for k,v in pairs(target_ply) do
		SimpleXPSetXPText(v,SimpleXPCalculateLevelToXP(level),string.upper(reason))
	end
...
ULXSETXP:addParam{type=ULib.cmds.StringArg,default="Cool Guy", hint="reason",ULib.cmds.optional}
