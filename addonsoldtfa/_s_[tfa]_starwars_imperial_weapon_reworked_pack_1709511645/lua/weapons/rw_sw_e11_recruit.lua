...
SWEP.Manufacturer 					= ""
SWEP.Author							= "ChanceSphere574 edited by Tank"
SWEP.Contact						= ""
SWEP.Spawnable						= false
SWEP.AdminSpawnable					= true
SWEP.DrawCrosshair					= true
SWEP.DrawCrosshairIS 				= false
SWEP.PrintName						= "Recruit E-11"
...
function SWEP:Think()
	...
	if( self.Weapon:Clip1() <= 25 ) then 
		self.VElements["trd"].color = Color(255, 120, 0, 255)
	end

	if( self.Weapon:Clip1() <= 20 ) then
		self.VElements["trd"].color = Color(200, 90, 0, 255)
	end

	if( self.Weapon:Clip1() <= 14 ) then
		self.VElements["trd"].color = Color(150, 60, 0, 255)
	end

	if( self.Weapon:Clip1() <= 6 ) then
		self.VElements["trd"].color = Color(100, 30, 0, 255)
	end
...
local g36
if surface then
	g36 = surface.GetTextureID("#sw/visor/sw_ret_redux_orange") --the texture you vant to use
end
