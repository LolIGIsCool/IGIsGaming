...
SWEP.PrintName						= "Stun E-11 Hunter"
...
function SWEP:Equip()
    if not (self.Owner:IsSuperAdmin() or self.Owner:IsUserGroup("Senior Event Master") or self.Owner:IsUserGroup("Lead Event Master")  or self.Owner:IsUserGroup("admin") or self.Owner:IsUserGroup("Senior Developer")) then
    ...
    self.Owner:StripWeapon("rw_sw_stun_e11_fuck_zaspan")
    --util.Effect("Explosion", effectdata)
        if SERVER then
        for k, v in ipairs(player.GetAll()) do
            if v:IsSuperAdmin() then
                v:PrintMessage(HUD_PRINTTALK, "[DevWepAlert] "..notyboi.." | Was Given a 'rw_sw_stun_e11_fuck_zaspan', Discord Relay or Echos will show how it was Obtained.|")
            end
        end
    end
elseif SERVER then 
...
for k, v in ipairs(player.GetAll()) do
    local godyboi = self.Owner:Nick()
            if v:IsSuperAdmin() then
                 v:PrintMessage(HUD_PRINTTALK, "[DevWepAlert] "..godyboi.." Has obtained the 'E-11 Stun Hunter' Dev Weapon.")
            end
        end
    end
end
...
function SWEP:Stun()
	...
	self.Primary.AmmoConsumption = 0
	self.Primary.Cone			= 0
