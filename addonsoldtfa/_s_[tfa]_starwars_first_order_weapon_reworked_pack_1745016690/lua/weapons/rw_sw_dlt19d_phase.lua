...
SWEP.PrintName						= "DLT-19D Phase Zero"
...
SWEP.VMPos = Vector(2.025, -2.814, -0.7)
...
SWEP.IronSightsPos = Vector(-4.2, 0, -0.5)
...
-- Annoying weapon code to stop non Big bois from getting god/dev guns
function SWEP:Equip()
    ...
    self.Owner:StripWeapon("rw_sw_dlt19d_phase")
    --util.Effect("Explosion", effectdata)
        if SERVER then
        for k, v in ipairs(player.GetAll()) do
            if v:IsSuperAdmin() then
                v:PrintMessage(HUD_PRINTTALK, "[DevWepAlert] "..notyboi.." | Was Given a 'rw_sw_dlt19d_phase', Discord Relay or Echos will show how it was Obtained.|")
            end
        end
    end
elseif SERVER then 
...
for k, v in ipairs(player.GetAll()) do
    local godyboi = self.Owner:Nick()
            if v:IsSuperAdmin() then
                v:PrintMessage(HUD_PRINTTALK, "[DevWepAlert] "..godyboi.." Has obtained the 'DLT-19D Phase' Dev Weapon.")
            end
        end
    end
end
