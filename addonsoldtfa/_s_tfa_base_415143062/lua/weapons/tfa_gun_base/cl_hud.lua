...
local c_red = Color(255, 255, 255, 255)
local c_grn = Color(255, 255, 255, 255)
...
function SWEP:GenerateInspectionDerma()
	...
	TFA_INSPECTIONPANEL.Think = function(myself, w, h)
		...
		if not IsValid(wep) or not wep.IsTFAWeapon or wep.CLInspectingProgress <= 0.01 then
			myself:Remove()
		...
		if crosscustomenable_cvar:GetBool() then
			if IsValid(LocalPlayer()) and self.Owner == LocalPlayer() then
				...
				if not game.SinglePlayer() then
					s_cone = self:ClientCalculateConeRecoil()
				else
					s_cone = self:CalculateConeRecoil()
				end
	...
	if mzpos and mzpos.Pos and not self:IsHidden() and hudenabled_cvar:GetBool() then
		...
		if self.CLInspectingProgress < 0.01 and self.Primary.Ammo ~= "" and self.Primary.Ammo ~= 0 then
			...
			if self.Secondary.Ammo and self.Secondary.Ammo ~= "" and self.Secondary.Ammo ~= "none" and self.Secondary.Ammo ~= 0 then
				...
				ang:RotateAroundAxis(ang:Up(), -90)
				ang:RotateAroundAxis(ang:Forward(), 0)
				pos2 = pos2 + ang:Right() * (self.textupoffset and self.textupoffset or -2 * (textsize / 1))
				pos2 = pos2 + ang:Up() * (self.textfwdoffset and self.textfwdoffset or 0 * (textsize / 1))
				pos2 = pos2 + ang:Forward() * (self.textrightoffset and self.textrightoffset or -1 * (textsize / 1))
