...
function SWEP:UpdateMuzzleAttachment()
	...
	if self.Akimbo then
		if self.Akimbo_Inverted then
			self.MuzzleAttachmentRaw = self.AnimCycle + 1
		else
			self.MuzzleAttachmentRaw = 2 - self.AnimCycle
		end

		if self.Akimbo_Inverted then
			self.ShellAttachmentRaw = self.AnimCycle + 3
		else
			self.ShellAttachmentRaw = 4 - self.AnimCycle
		end
	end
...
function SWEP:UpdateViewModel()
	if not self.OwnerViewModel and self.Owner.GetViewModel then
...
function SWEP:CanChamber()
	if self.DisableChamberingNew ~= nil then
		...
		if self.Category == "TFA Pistols" and self.HoldType == "revolver" then
			self.Revolver = true
		end
...
function SWEP:GetRPM()
	...
	if self.Primary.RPM then return self.Primary.RPM end
...
function SWEP:GetAmmoForceMultiplier()
	...
	if (am == "pistol") then
		--M1911 .45 ACP penetrates 2 inches of concrete or so max
		--To compensate for the detection algorithm giving revolvers super penetration power because of recoil
		--P90 penetrates 3 inches of the ol' crete, and is very capable at penetrating wood or sheet metal.  It can go through about 6 inches of wood or about 2.28 inches of light metal (most in-game is aluminum or light steel.)
		--50 BMG
		--.308
		--Gotta compensate for
		return 0.4
	elseif (am == "357") then
		return 0.6
	elseif (am == "smg1") then
...
function SWEP:CanDustEffect( matv )
	if self.Callback.CanDustEffect then
		local val = self.Callback.CanDustEffect(self, matv )
		if val then return val end
	end
...
function SWEP:CanSparkEffect(matv)
	if self.Callback.CanSparkEffect then
		local val = self.Callback.CanSparkEffect(self, matv)
		if val then return val end
	end
