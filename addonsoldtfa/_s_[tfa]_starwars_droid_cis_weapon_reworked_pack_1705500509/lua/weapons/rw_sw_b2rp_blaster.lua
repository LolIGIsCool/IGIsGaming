...
SWEP.WElements = {
	["b2rp"] = { type = "Model", model = "models/hunter/plates/plate.mdl", bone = "ValveBiped.Bip01_R_Hand", rel = "", pos = Vector(7.9, 1, 1), angle = Angle(-8, 0, 180), size = Vector(0.1, 0.1, 0.1), color = Color(255, 255, 255, 255), surpresslightning = false, material = "", skin = 0, bodygroup = {} }
}
...
function SWEP:Think()
	if (self.Weapon:Clip1() == 0) and SERVER then
		if (CurrentTimer == 50) then 
			CurrentTimer = 0
			self.Weapon:SetClip1( self.Weapon:Clip1() + 20 )
		else
