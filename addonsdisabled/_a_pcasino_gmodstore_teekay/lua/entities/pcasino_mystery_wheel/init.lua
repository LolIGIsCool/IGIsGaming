...
function ENT:Use(ply)
	...
	if tobool(self.data.buySpin.buy) then
		local spinCost = tonumber(self.data.buySpin.cost)
		if PerfectCasino.Config.CanAfford(ply, spinCost) then
			PerfectCasino.Config.AddMoney(ply, -spinCost)
			hook.Run("pCasinoOnMysteryWheelBet", ply, self, spinCost)

			PerfectCasino.Core.Msg(string.format(PerfectCasino.Translation.Chat.UsedPaidSpin, PerfectCasino.Config.FormatMoney(spinCost)), ply)
