...
hook.Add( "entity_killed", "PlayerDeath", function( tbl )
	local attacker = Entity( tbl.entindex_attacker )
	local inflictor = Entity( tbl.entindex_inflictor )
	local victim = Entity( tbl.entindex_killed )
	if !IsValid( victim ) or !IsValid( attacker ) then return end
	if IsValid( inflictor ) and inflictor:IsPlayer() and inflictor:GetActiveWeapon() and IsValid( inflictor:GetActiveWeapon() ) then
	...
	GAMEMODE:AddDeathNotice( attacker:IsPlayer() and attacker:Nick() or "#" .. attacker:GetClass(),
		attacker:IsPlayer() and attacker:Team() or -1,
		inflictor,
		victim:IsPlayer() and victim:Nick() or "#" .. victim:GetClass(),
		victim:IsPlayer() and victim:Team() or -1, true )
end )
...
-----------------------------------------------------------]]
function GM:AddDeathNotice( Attacker, team1, Inflictor, Victim, team2, basecopy )
	if !basecopy then return end
	local Death = {}
	Death.time		= CurTime()
