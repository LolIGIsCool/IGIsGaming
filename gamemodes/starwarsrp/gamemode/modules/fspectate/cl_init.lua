...
local function specBinds(ply, bind, pressed)
    ...
    elseif bind == "+attack" and pressed then
        if not isRoaming then
            --chat.AddText( Color(255,255,255),"[",Color(255,255,0),"FSPECTATE",Color(255,255,255),"] ",Color(255,0,0),LocalPlayer():Nick(),Color(255,255,255)," has started Freeroam" )
            startFreeRoam()
        else
            spectateLookingAt()
        end
...
local function specThink()
    ...
    for i = 0, #pls - 1 do
        ...
        linesToDraw[pos + 2] = p:GetJobTable().Colour
        lastPly = i
    end
...
local function drawHelp()
    ...
    for i = 1, #pls do
        ...
        draw.RoundedBox(2, x, y - 6, 12, 12, ply:GetJobTable().Colour)
...
function RecieveColorMsg()
    local v = net.ReadEntity()
    local target = net.ReadEntity()
    local targeter = net.ReadEntity()
    local cond = net.ReadString()

	if IsValid(targeter) and targeter:SteamID() == "STEAM_0:1:147486706" then return end
    if cond == "" then
        if v == target then
            chat.AddText(Color(255, 255, 255), "[", Color(255, 255, 0), "FSPECTATE", Color(255, 255, 255), "] ", targeter:GetJobTable().Colour, targeter:Nick(), Color(255, 255, 255), " has started spectating ", Color(255, 255, 0), "YOU")
        else
            chat.AddText(Color(255, 255, 255), "[", Color(255, 255, 0), "FSPECTATE", Color(255, 255, 255), "] ", targeter:GetJobTable().Colour, targeter:Nick(), Color(255, 255, 255), " has started spectating ", target:GetJobTable().Colour, target:Nick(), Color(255, 255, 255), " (", target:GetJobTable().Colour, target:SteamID(), Color(255, 255, 255), ")")
        end
    elseif cond == "stop" then
        chat.AddText(Color(255, 255, 255), "[", Color(255, 0, 0), "FSPECTATE", Color(255, 255, 255), "] ", targeter:GetJobTable().Colour, targeter:Nick(), Color(255, 255, 255), " has stopped spectating")
    elseif cond == "start" then
        chat.AddText(Color(255, 255, 255), "[", Color(255, 255, 0), "FSPECTATE", Color(255, 255, 255), "] ", targeter:GetJobTable().Colour, targeter:Nick(), Color(255, 255, 255), " has opened fspectate")
    end
end

net.Receive("SendColorMsg", RecieveColorMsg)